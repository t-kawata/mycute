...
> 本テストでは、各操作の後に **Stats API (`GET /v1/cubes/stats`) を呼び出し、統計情報が正しく記録されているか**を必ず確認してください。
> 
> **確認ポイント**:
> - `MemoryGroup` ごとに正しくグループ化されているか
> - `CubeModelStat`: トークン使用量 (`InputTokens`, `OutputTokens`) が正確か
> - `CubeContributor`: 貢献者と使用モデルの組み合わせが正しいか
> - `ActionType`: Training (Absorb/Memify) と Search が区別されているか
> 
> **統計情報に誤りがある場合は、他のテスト項目よりも統計情報の修正を最優先してください。**
> これは「この Cube はどの分野にどれだけ詳しいか」をユーザーが判断するための根幹情報であり、mycute サービスの商品価値に直結する重要事項です。
