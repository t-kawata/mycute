# Declorch — Agent Instructions

## Project Overview
Declorch is an open-source Agent Operating System written in Rust (14 crates).
- Config: `~/.declorch/config.toml`
- Default API: `http://127.0.0.1:4200`
- CLI binary: `target/release/declorch.exe` (or `target/debug/declorch.exe`)
...
```bash
cargo build --release -p declorch-cli
```

#### Step 3: Start daemon with required API keys
```bash
GROQ_API_KEY=<key> target/release/declorch.exe start &
...
## Architecture Notes
- **Don't touch `declorch-cli`** — user is actively building the interactive CLI
...
## Common Gotchas
- `declorch.exe` may be locked if daemon is running — use `--lib` flag or kill daemon first
