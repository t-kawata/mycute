//! DeclorchKernel — assembles all subsystems and provides the main API.
...
#[cfg(feature = "memory")]
use declorch_memory::MemorySubstrate;
use declorch_runtime::agent_loop::{
    run_agent_loop, run_agent_loop_streaming, strip_provider_prefix, AgentLoopResult,
};
use declorch_runtime::audit::AuditLog;
use declorch_runtime::drivers;
use declorch_runtime::kernel_handle::{self, KernelHandle};
use declorch_runtime::llm_driver::{
    CompletionRequest, CompletionResponse, DriverConfig, LlmDriver, LlmError, StreamEvent,
};
use declorch_runtime::python_runtime::{self, PythonConfig};
use declorch_runtime::routing::ModelRouter;
use declorch_runtime::sandbox::{SandboxConfig, WasmSandbox};
use declorch_runtime::tool_runner::builtin_tool_definitions;
use declorch_types::agent::*;
use declorch_types::capability::Capability;
use declorch_types::config::{KernelConfig, OutputFormat};
use declorch_types::error::DeclorchError;
use declorch_types::event::*;
use declorch_types::memory::Memory;
use declorch_types::tool::ToolDefinition;
...
/// The main Declorch kernel — coordinates all subsystems.
...
pub struct DeclorchKernel {
    ...
    pub model_catalog: std::sync::RwLock<declorch_runtime::model_catalog::ModelCatalog>,
    /// Skill registry for plugin skills (RwLock for hot-reload on install/uninstall).
    pub skill_registry: std::sync::RwLock<declorch_skills::registry::SkillRegistry>,
    ...
    pub mcp_connections: tokio::sync::Mutex<Vec<declorch_runtime::mcp::McpConnection>>,
    ...
    pub a2a_task_store: declorch_runtime::a2a::A2aTaskStore,
    /// Discovered external A2A agent cards.
    pub a2a_external_agents: std::sync::Mutex<Vec<(String, declorch_runtime::a2a::AgentCard)>>,
    /// Web tools context (multi-provider search + SSRF-protected fetch + caching).
    pub web_ctx: declorch_runtime::web_search::WebToolsContext,
    /// Browser automation manager (Playwright bridge sessions).
    pub browser_ctx: declorch_runtime::browser::BrowserManager,
    /// Media understanding engine (image description, audio transcription).
    pub media_engine: declorch_runtime::media_understanding::MediaEngine,
    /// Text-to-speech engine.
    pub tts_engine: declorch_runtime::tts::TtsEngine,
    ...
    pub embedding_driver:
        Option<Arc<dyn declorch_runtime::embedding::EmbeddingDriver + Send + Sync>>,
    /// Hand registry — curated autonomous capability packages.
    pub hand_registry: declorch_hands::registry::HandRegistry,
    /// Credential resolver — vault → dotenv → env var priority chain.
    pub credential_resolver: std::sync::Mutex<declorch_extensions::credentials::CredentialResolver>,
    /// Extension/integration registry (bundled MCP templates + install state).
    pub extension_registry: std::sync::RwLock<declorch_extensions::registry::IntegrationRegistry>,
    /// Integration health monitor.
    pub extension_health: declorch_extensions::health::HealthMonitor,
    /// Effective MCP server list (manual config + extension-installed, merged at boot).
    pub effective_mcp_servers: std::sync::RwLock<Vec<declorch_types::config::McpServerConfigEntry>>,
    ...
    pub bindings: std::sync::Mutex<Vec<declorch_types::config::AgentBinding>>,
    /// Broadcast configuration.
    pub broadcast: declorch_types::config::BroadcastConfig,
    /// Auto-reply engine.
    pub auto_reply_engine: crate::auto_reply::AutoReplyEngine,
    /// Plugin lifecycle hook registry.
    pub hooks: declorch_runtime::hooks::HookRegistry,
    /// Persistent process manager for interactive sessions (REPLs, servers).
    pub process_manager: Arc<declorch_runtime::process_manager::ProcessManager>,
    /// OFP peer registry — tracks connected peers (OnceLock for safe init after Arc creation).
    pub peer_registry: OnceLock<declorch_wire::PeerRegistry>,
    /// OFP peer node — the local networking node (OnceLock for safe init after Arc creation).
    pub peer_node: OnceLock<Arc<declorch_wire::PeerNode>>,
    ...
    pub channel_adapters:
        dashmap::DashMap<String, Arc<dyn declorch_channels::types::ChannelAdapter>>,
    /// Hot-reloadable default model override (set via config hot-reload, read at agent spawn).
    pub default_model_override:
        std::sync::RwLock<Option<declorch_types::config::DefaultModelConfig>>,
    ...
    pub fallback_providers_override:
        std::sync::RwLock<Option<Vec<declorch_types::config::FallbackProviderConfig>>>,
    ...
    self_handle: OnceLock<Weak<DeclorchKernel>>,
}

/// Bounded in-memory delivery receipt tracker.
/// Stores up to `MAX_RECEIPTS` most recent delivery receipts per agent.
pub struct DeliveryTracker {
    receipts: dashmap::DashMap<AgentId, Vec<declorch_channels::types::DeliveryReceipt>>,
}
...
impl DeliveryTracker {
    ...
    pub fn record(&self, agent_id: AgentId, receipt: declorch_channels::types::DeliveryReceipt) {
    ...
    pub fn get_receipts(
        ...
        agent_id: AgentId,
        limit: usize,
    ) -> Vec<declorch_channels::types::DeliveryReceipt> {
        self.receipts
            .get(&agent_id)
    ...
    pub fn sent_receipt(
        channel: &str,
        recipient: &str,
    ) -> declorch_channels::types::DeliveryReceipt {
        declorch_channels::types::DeliveryReceipt {
            message_id: uuid::Uuid::new_v4().to_string(),
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: declorch_channels::types::DeliveryStatus::Sent,
    ...
    pub fn failed_receipt(
        ...
        declorch_channels::types::DeliveryReceipt {
            ...
            channel: channel.to_string(),
            recipient: Self::sanitize_recipient(recipient),
            status: declorch_channels::types::DeliveryStatus::Failed,
            timestamp: chrono::Utc::now(),
...
/// `~/.declorch/workspaces/{name}/` regardless of where the user pointed the
...
fn ensure_state_dir(state_dir: &Path, workspace: &Path) -> KernelResult<()> {
    for subdir in &["sessions", "logs", "memory"] {
        std::fs::create_dir_all(state_dir.join(subdir)).map_err(|e| {
            KernelError::Declorch(DeclorchError::Internal(format!(
...
fn append_daily_memory_log(state_dir: &Path, response: &str) {
    ...
    let summary = declorch_types::truncate_str(trimmed, 500);
...
fn read_identity_file(state_dir: &Path, filename: &str) -> Option<String> {
    ...
    if content.len() > MAX_IDENTITY_FILE_BYTES {
        Some(declorch_types::truncate_str(&content, MAX_IDENTITY_FILE_BYTES).to_string())
    } else {
        Some(content)
    }
}
...
impl DeclorchKernel {
    ...
    fn fetch_copilot_models(declorch_dir: &Path) -> Result<Vec<String>, String> {
        use declorch_runtime::drivers::copilot;

        let tokens = copilot::PersistedTokens::load(declorch_dir)
    ...
    pub fn boot_with_config(mut config: KernelConfig) -> KernelResult<Self> {
        ...
        use declorch_types::config::KernelMode;
        ...
        match config.mode {
            KernelMode::Stable => {
                info!("Booting Declorch kernel in STABLE mode — conservative defaults enforced");
            }
            KernelMode::Dev => {
                warn!("Booting Declorch kernel in DEV mode — experimental features enabled");
            }
            KernelMode::Default => {
                info!("Booting Declorch kernel...");
            }
        }
        ...
        let db_path = config
            ...
            .sqlite_path
            .clone()
            .unwrap_or_else(|| config.data_dir.join("declorch.db"));
        #[cfg(feature = "memory")]
        let memory = Arc::new(
        ...
        let credential_resolver = {
            ...
            let vault = if vault_path.exists() {
                let mut v = declorch_extensions::vault::CredentialVault::new(vault_path);
                match v.unlock() {
                    Ok(()) => {
            ...
            declorch_extensions::credentials::CredentialResolver::new(vault, Some(&dotenv_path))
        };
        ...
        let driver: Arc<dyn LlmDriver> = if driver_chain.len() > 1 {
            Arc::new(declorch_runtime::drivers::fallback::FallbackDriver::with_models(model_chain))
        ...
        let metering = Arc::new(MeteringEngine::new(Arc::new(
            declorch_memory::usage::UsageStore::new(memory.usage_conn()),
        )));
        ...
        let mut model_catalog = declorch_runtime::model_catalog::ModelCatalog::new();
        ...
        // Load user's custom models from ~/.declorch/custom_models.json
        ...
        if declorch_runtime::drivers::copilot::copilot_auth_available(&config.home_dir) {
        ...
        let mut skill_registry = declorch_skills::registry::SkillRegistry::new(skills_dir);
        ...
        let hand_registry = declorch_hands::registry::HandRegistry::new();
        ...
        // Hands installed via `declorch hand install <path>` are persisted to
        ...
        let mut extension_registry =
            declorch_extensions::registry::IntegrationRegistry::new(&config.home_dir);
        ...
        let health_config = declorch_extensions::health::HealthMonitorConfig {
        ...
        let extension_health = declorch_extensions::health::HealthMonitor::new(health_config);
        ...
        let web_cache = Arc::new(declorch_runtime::web_cache::WebCache::new(cache_ttl));
        let web_ctx = declorch_runtime::web_search::WebToolsContext {
            search: declorch_runtime::web_search::WebSearchEngine::new(
                config.web.clone(),
                web_cache.clone(),
            ),
            fetch: declorch_runtime::web_fetch::WebFetchEngine::new(
        ...
        let embedding_driver: Option<
            Arc<dyn declorch_runtime::embedding::EmbeddingDriver + Send + Sync>,
        > = {
            use declorch_runtime::embedding::create_embedding_driver;
        ...
        let browser_ctx = declorch_runtime::browser::BrowserManager::new(config.browser.clone());

        // Initialize media understanding engine
        let media_engine =
            declorch_runtime::media_understanding::MediaEngine::new(config.media.clone());
        ...
        let tts_engine = declorch_runtime::tts::TtsEngine::new(config.tts.clone()).with_base_urls(
        ...
        let kernel = Self {
            ...
            a2a_task_store: declorch_runtime::a2a::A2aTaskStore::default(),
            ...
            auto_reply_engine,
            hooks: declorch_runtime::hooks::HookRegistry::new(),
            process_manager: Arc::new(declorch_runtime::process_manager::ProcessManager::new(5)),
        ...
        // for every subsequent install/upsert/reload.
        {
            ...
            for (hand_id, toml_content, _skill) in declorch_hands::bundled::bundled_hands() {
                ...
                audit_log_initial.record(
                    "kernel",
                    declorch_runtime::audit::AuditAction::ConfigChange,
                ...
                for entry in agents {
                    ...
                    if toml_path.exists() {
                        match std::fs::read_to_string(&toml_path) {
                            Ok(toml_str) => {
                                match toml::from_str::<declorch_types::agent::AgentManifest>(
                                    &toml_str,
                                ) {
                                    Ok(disk_manifest) => {
        ...
        // Issue #1140: auto-spawn agents from `~/.declorch/agents/<name>/agent.toml`
            ...
            if agents_dir.is_dir() {
                ...
                if let Ok(entries) = std::fs::read_dir(&agents_dir) {
                    for entry in entries.flatten() {
                        ...
                        let mut manifest: declorch_types::agent::AgentManifest =
                        ...
                        match kernel.spawn_agent(manifest) {
                            Ok(id) => {
                                ...
                                info!(
                                    agent = %dir_name,
                                    id = %id,
                                    "Auto-spawned agent from ~/.declorch/agents"
                                );
                            }
                            Err(e) => {
                                tracing::warn!(
                ...
                if auto_spawned > 0 {
                    info!("Auto-spawned {auto_spawned} agent(s) from ~/.declorch/agents");
                }
            }
        }
        ...
        if kernel.registry.list().is_empty() {
            ...
            let manifest = AgentManifest {
                ...
                model: declorch_types::agent::ModelConfig {
        ...
        info!("Declorch kernel booted successfully");
        Ok(kernel)
    }
    ...
    pub fn spawn_agent_with_parent(
        ...
        let session = self
            ...
            .map_err(KernelError::Declorch)?;
        #[cfg(not(feature = "memory"))]
        let session = declorch_types::session::Session {
            id: declorch_types::agent::SessionId(uuid::Uuid::new_v4()),
            agent_id,
            messages: Vec::new(),
        ...
        // Agent private state always lives under ~/.declorch/workspaces/{name}/.
        ...
        self.audit_log.record(
            agent_id.to_string(),
            declorch_runtime::audit::AuditAction::AgentSpawn,
    ...
    pub fn verify_signed_manifest(&self, signed_json: &str) -> KernelResult<String> {
        let signed: declorch_types::manifest_signing::SignedManifest =
            serde_json::from_str(signed_json).map_err(|e| {
                KernelError::Declorch(declorch_types::error::DeclorchError::Config(format!(
    ...
    pub async fn send_message_with_blocks(
        ...
        agent_id: AgentId,
        message: &str,
        blocks: Vec<declorch_types::message::ContentBlock>,
    ) -> KernelResult<AgentLoopResult> {
        let handle: Option<Arc<dyn KernelHandle>> = self
            .self_handle
            .get()
    ...
    pub async fn send_message_with_handle_and_blocks(
        ...
        message: &str,
        kernel_handle: Option<Arc<dyn KernelHandle>>,
        content_blocks: Option<Vec<declorch_types::message::ContentBlock>>,
        sender_id: Option<String>,
        sender_name: Option<String>,
    ) -> KernelResult<AgentLoopResult> {
        ...
        let entry = self.registry.get(agent_id).ok_or_else(|| {
            KernelError::Declorch(DeclorchError::AgentNotFound(agent_id.to_string()))
        })?;
        ...
        match result {
            Ok(result) => {
                ...
                self.audit_log.record(
                    agent_id.to_string(),
                    declorch_runtime::audit::AuditAction::AgentMessage,
                    format!(
    ...
    pub fn send_message_streaming(
                    ...
                    Ok(result) => {
                        ...
                        let _ = tx
                            .send(StreamEvent::ContentComplete {
                                stop_reason: declorch_types::message::StopReason::EndTurn,
        ...
        let mut session = self
            ...
            .map_err(KernelError::Declorch)?
            .unwrap_or_else(|| declorch_types::session::Session {
                id: entry.session_id,
                agent_id,
                messages: Vec::new(),
                context_window_tokens: 0,
        ...
        #[cfg(not(feature = "memory"))]
        let mut session = declorch_types::session::Session {
            id: entry.session_id,
            agent_id,
            messages: Vec::new(),
            context_window_tokens: 0,
        ...
        let needs_compact = {
            use declorch_runtime::compactor::{
        ...
        // ~/.declorch/workspaces/{name}/; the user-facing workspace defaults
        ...
        // Loading order: bundled → global (~/.declorch/skills) → workspace skills.
        ...
        // Build the structured system prompt via prompt_builder
        {
            ...
            #[cfg(feature = "memory")]
            let canonical_context: Option<String> = self
                .memory
                .canonical_context(agent_id, None)
                .ok()
                .and_then(|(s, _)| s);
            #[cfg(not(feature = "memory"))]
            let canonical_context: Option<String> = None;

            let prompt_ctx = declorch_runtime::prompt_builder::PromptContext {
                ...
                workspace_context: manifest.workspace.as_ref().map(|w| {
                    let mut ws_ctx =
                        declorch_runtime::workspace_context::WorkspaceContext::detect(w);
                ...
                context_md: manifest.workspace.as_ref().and_then(|w| {
                    declorch_runtime::agent_context::load_context_md(w, manifest.cache_context)
                }),
            };
            manifest.model.system_prompt =
                declorch_runtime::prompt_builder::build_system_prompt(&prompt_ctx);
            ...
            if let Some(cc_msg) =
                declorch_runtime::prompt_builder::build_canonical_context_message(&prompt_ctx)
            {
        ...
        let message_owned = if let Some(link_ctx) =
            declorch_runtime::link_understanding::build_link_context(message, &self.config.links)
        {
        ...
        let handle = tokio::spawn(async move {
            ...
            let phase_cb: declorch_runtime::agent_loop::PhaseCallback =
                std::sync::Arc::new(move |phase| {
                    use declorch_runtime::agent_loop::LoopPhase;
            ...
            match result {
                Ok(result) => {
                    ...
                    let _ = kernel_clone
                        .metering
                        .record(&declorch_types::usage::UsageRecord {
                            agent_id,
                            model: model.clone(),
    ...
    async fn execute_wasm_agent(
        ...
        Ok(AgentLoopResult {
            response,
            total_usage: declorch_types::message::TokenUsage {
                input_tokens: 0,
                output_tokens: 0,
            },
            iterations: 1,
    ...
    async fn execute_llm_agent(
        ...
        // Lazy backfill: create state_dir and workspace for existing agents.
        // Private state lives under ~/.declorch/workspaces/{name}/. User-facing
        ...
        let is_stable = self.config.mode == declorch_types::config::KernelMode::Stable;
        ...
        if is_stable {
            ...
            let probe = CompletionRequest {
                ...
                messages: vec![declorch_types::message::Message::user(message)],
                tools: tools.clone(),
        ...
        let _ = self.metering.record(&declorch_types::usage::UsageRecord {
            agent_id,
            model: model.clone(),
            input_tokens: result.total_usage.input_tokens,
            output_tokens: result.total_usage.output_tokens,
        ...
        match self.config.usage_footer {
            declorch_types::config::UsageFooterMode::Off => {
                result.cost_usd = None;
            }
            declorch_types::config::UsageFooterMode::Cost
            | declorch_types::config::UsageFooterMode::Full => {
                result.cost_usd = if cost > 0.0 { Some(cost) } else { None };
            }
            declorch_types::config::UsageFooterMode::Tokens => {
    ...
    pub fn reset_session(&self, agent_id: AgentId) -> KernelResult<()> {
        ...
        let new_session = self
            ...
            .map_err(KernelError::Declorch)?;
        #[cfg(not(feature = "memory"))]
        let new_session = declorch_types::session::Session {
            id: declorch_types::agent::SessionId(uuid::Uuid::new_v4()),
            agent_id,
            messages: Vec::new(),
            context_window_tokens: 0,
    ...
    pub fn create_agent_session(
        ...
        #[cfg(not(feature = "memory"))]
        {
            Err(KernelError::Declorch(DeclorchError::Internal(
                "Memory subsystem not available".to_string(),
            )))
        }
    }
    ...
    pub fn switch_agent_session(
        ...
        let session = self
            ...
            .map_err(KernelError::Declorch)?
            .ok_or_else(|| {
                KernelError::Declorch(DeclorchError::Internal("Session not found".to_string()))
            })?;

        if session.agent_id != agent_id {
            return Err(KernelError::Declorch(DeclorchError::Internal(
    ...
    fn save_session_summary(
        ...
        agent_id: AgentId,
        entry: &AgentEntry,
        session: &declorch_types::session::Session,
    ) {
        use declorch_types::message::{MessageContent, Role};
        ...
        let summary = format!(
            ...
            topics
                ...
                .enumerate()
                .map(|(i, t)| {
                    let truncated = declorch_types::truncate_str(t, 200);
        ...
        if !skills.is_empty() {
            ...
            for name in &skills {
                if !known.contains(name) {
                    return Err(KernelError::Declorch(DeclorchError::Internal(format!(
                        "Unknown skill: {name}"
                    ))));
                }
            }
        ...
        if !servers.is_empty() {
            if let Ok(mcp_tools) = self.mcp_tools.lock() {
                ...
                for tool in mcp_tools.iter() {
                    if let Some(s) = declorch_runtime::mcp::extract_mcp_server(&tool.name) {
                        known_servers.insert(s.to_string());
                    }
                }
                for name in &servers {
                    let normalized = declorch_runtime::mcp::normalize_name(name);
    ...
    pub fn session_usage_cost(&self, agent_id: AgentId) -> KernelResult<(u64, u64, f64)> {
        ...
        let session = self
            ...
            .map_err(KernelError::Declorch)?;
        #[cfg(not(feature = "memory"))]
        let session: Option<declorch_types::session::Session> = None;
        ...
        let (input_tokens, output_tokens) = session
            .map(|s| {
                ...
                for msg in &s.messages {
                    ...
                    let tokens = len / 4;
                    match msg.role {
                        declorch_types::message::Role::User => input += tokens,
                        declorch_types::message::Role::Assistant => output += tokens,
                        declorch_types::message::Role::System => input += tokens,
                    }
                }
                (input, output)
            })
    ...
    pub async fn compact_agent_session(&self, agent_id: AgentId) -> KernelResult<String> {
        use declorch_runtime::compactor::{compact_session, needs_compaction, CompactionConfig};
        ...
        let result = compact_session(driver, &model, &session, &config)
            .await
            .map_err(|e| KernelError::Declorch(DeclorchError::Internal(e)))?;
        ...
        let (repaired_messages, repair_stats) =
            declorch_runtime::session_repair::validate_and_repair_with_stats(&result.kept_messages);
    ...
    pub fn context_report(
        &self,
        agent_id: AgentId,
    ) -> KernelResult<declorch_runtime::compactor::ContextReport> {
        use declorch_runtime::compactor::generate_context_report;
    ...
    pub fn kill_agent(&self, agent_id: AgentId) -> KernelResult<()> {
        ...
        self.audit_log.record(
            agent_id.to_string(),
            declorch_runtime::audit::AuditAction::AgentKill,
    ...
    pub fn activate_hand(
    ...
    ) -> KernelResult<declorch_hands::HandInstance> {
        use declorch_hands::HandError;

        let def = self
            .hand_registry
            .get_definition(hand_id)
            .ok_or_else(|| {
                KernelError::Declorch(DeclorchError::AgentNotFound(format!(
        ...
        let instance = self
            ...
            .map_err(|e| match e {
                HandError::AlreadyActive(id) => KernelError::Declorch(DeclorchError::Internal(
                    format!("Hand already active: {id}"),
                )),
                other => KernelError::Declorch(DeclorchError::Internal(other.to_string())),
            })?;
        ...
        let mut manifest = AgentManifest {
            ...
            exec_policy: if def.tools.iter().any(|t| t == "shell_exec") {
                Some(declorch_types::config::ExecPolicy {
                    mode: declorch_types::config::ExecSecurityMode::Full,
        ...
        let resolved = declorch_hands::resolve_settings(&def.settings, &instance.config);
        ...
        for req in &def.requires {
            match req.requirement_type {
                declorch_hands::RequirementType::ApiKey
                | declorch_hands::RequirementType::EnvVar
        ...
        let saved_crons: Vec<declorch_types::scheduler::CronJob> = old_agent_id
        ...
        self.hand_registry
            ...
            .map_err(|e| KernelError::Declorch(DeclorchError::Internal(e.to_string())))?;
    ...
    pub fn list_bindings(&self) -> Vec<declorch_types::config::AgentBinding> {
        self.bindings
            .lock()
            .unwrap_or_else(|e| e.into_inner())
            .clone()
    ...
    pub fn add_binding(&self, binding: declorch_types::config::AgentBinding) {
    ...
    pub fn remove_binding(&self, index: usize) -> Option<declorch_types::config::AgentBinding> {
        let mut bindings = self.bindings.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn apply_hot_actions(
        ...
        new_config: &declorch_types::config::KernelConfig,
    ) {
    ...
    pub fn register_trigger(
        ...
        if self.registry.get(agent_id).is_none() {
            return Err(KernelError::Declorch(DeclorchError::AgentNotFound(
                agent_id.to_string(),
            )));
        }
        Ok(self
    ...
    pub async fn run_workflow(
        ...
        let run_id = self
            ...
            .await
            .ok_or_else(|| {
                KernelError::Declorch(DeclorchError::Internal("Workflow not found".to_string()))
            })?;
        ...
        .map_err(|e| {
            KernelError::Declorch(DeclorchError::Internal(format!("Workflow failed: {e}")))
        })?;
    ...
    pub fn start_background_agents(self: &Arc<Self>) {
        ...
        let state_path = self.config.home_dir.join("hand_state.json");
        let saved_hands = declorch_hands::registry::HandRegistry::load_state(&state_path);
        ...
        let mut bg_agents: Vec<(declorch_types::agent::AgentId, String, ScheduleMode)> = Vec::new();
        ...
        // warnings only fire for providers the operator actually configured.
        {
            ...
            tokio::spawn(async move {
                ...
                for (provider_id, base_url) in &local_providers {
                    let result =
                        declorch_runtime::provider_health::probe_provider(provider_id, base_url)
                            .await;
                    if result.reachable {
                        info!(
        ...
        // One-shot migration of legacy shared-memory `__declorch_schedules`
        ...
        if let Some(ref a2a_config) = self.config.a2a {
            if a2a_config.enabled && !a2a_config.external_agents.is_empty() {
                ...
                tokio::spawn(async move {
                    let discovered = declorch_runtime::a2a::discover_external_agents(&agents).await;
    ...
    async fn start_ofp_node(self: &Arc<Self>) {
        use declorch_wire::{PeerConfig, PeerNode, PeerRegistry};
        ...
        let node_name = gethostname().unwrap_or_else(|| "declorch-node".to_string());
        ...
        let handle: Arc<dyn declorch_wire::peer::PeerHandle> = self.self_arc();
    ...
    /// Migrate legacy `__declorch_schedules` shared-memory entries into the
    /// real cron scheduler.
    ///
    ...
    pub(crate) fn migrate_shared_memory_schedules(&self) {
        #[cfg(feature = "memory")]
        {
            ...
            const LEGACY_KEY: &str = "__declorch_schedules";
            const MARKER_KEY: &str = "__declorch_schedules_migrated_v1";

            let shared = shared_memory_agent_id();

            // Idempotency: if marker is already set, don't re-read.
            if let Ok(Some(Value::Bool(true))) =
                self.memory.structured_get(shared, MARKER_KEY)
            {
                return;
            }

            let entries: Vec<Value> = match self.memory.structured_get(shared, LEGACY_KEY) {
                Ok(Some(Value::Array(arr))) => arr,
                Ok(_) => {
                    let _ =
                        self.memory
                            .structured_set(shared, MARKER_KEY, Value::Bool(true));
                    return;
                }
                Err(e) => {
            ...
            info!(
                ...
                skipped,
                total = entries.len(),
                "Migrated legacy __declorch_schedules entries to cron scheduler"
            );

            if let Err(e) =
                self.memory
                    .structured_set(shared, LEGACY_KEY, Value::Array(Vec::new()))
            {
    ...
    pub fn shutdown(&self) {
        info!("Shutting down Declorch kernel...");
        ...
        info!(
            "Declorch kernel shut down ({} agents preserved)",
    ...
    fn referenced_providers(&self) -> std::collections::HashSet<String> {
        ...
        let fb_override = self
            ...
            .read()
            .unwrap_or_else(|e: std::sync::PoisonError<_>| e.into_inner());
        let fb_iter: &[declorch_types::config::FallbackProviderConfig] = fb_override
            .as_deref()
        ...
        let channel_overrides: [Option<&declorch_types::config::ChannelOverrides>; 43] = [
    ...
    fn resolve_driver(&self, manifest: &AgentManifest) -> KernelResult<Arc<dyn LlmDriver>> {
        ...
        let fb_override_guard = self
            ...
            .read()
            .unwrap_or_else(|e: std::sync::PoisonError<_>| e.into_inner());
        let effective_fallbacks: &[declorch_types::config::FallbackProviderConfig] =
            fb_override_guard
                .as_deref()
                .unwrap_or(&self.config.fallback_providers);
        ...
        let mut chain: Vec<(
            std::sync::Arc<dyn declorch_runtime::llm_driver::LlmDriver>,
            String,
        ...
        if chain.len() > 1 {
            return Ok(Arc::new(
                declorch_runtime::drivers::fallback::FallbackDriver::with_models(chain),
            ));
        }
    ...
    async fn connect_mcp_servers(self: &Arc<Self>) {
        use declorch_runtime::mcp::{McpConnection, McpServerConfig, McpTransport};
        use declorch_types::config::McpTransportEntry;
    ...
    fn available_tools_with_registry(
        &self,
        agent_id: AgentId,
        skill_snapshot: Option<&declorch_skills::registry::SkillRegistry>,
    ) -> Vec<ToolDefinition> {
        ...
        if let Ok(mcp_tools) = self.mcp_tools.lock() {
            let mcp_candidates: Vec<ToolDefinition> = if mcp_allowlist.is_empty() {
                ...
                let normalized: Vec<String> = mcp_allowlist
                    .iter()
                    .map(|s| declorch_runtime::mcp::normalize_name(s))
                    .collect();
                mcp_tools
                    .iter()
                    .filter(|t| {
                        declorch_runtime::mcp::extract_mcp_server(&t.name)
        ...
        let exec_blocks_shell = entry.as_ref().is_some_and(|e| {
            e.manifest
                .exec_policy
                .as_ref()
                .is_some_and(|p| p.mode == declorch_types::config::ExecSecurityMode::Deny)
        });
    ...
    pub fn reload_skills(&self) {
        ...
        let mut fresh = declorch_skills::registry::SkillRegistry::new(skills_dir);
    ...
    fn build_skill_summary_from(
        registry: &declorch_skills::registry::SkillRegistry,
        ...
        // Issue #1038: skill directories (e.g. ~/.declorch/skills/) live OUTSIDE
                ...
                if let Some(ref ctx) = skill.manifest.prompt_context {
                    if !ctx.is_empty() {
                        let is_bundled = matches!(
                            ...
                            Some(declorch_skills::SkillSource::Bundled)
                        );
                        if is_bundled {
    ...
    pub async fn cron_run_job(
        self: &Arc<Self>,
        job: &declorch_types::scheduler::CronJob,
    ) -> Result<String, String> {
        use declorch_types::scheduler::CronAction;
...
fn apply_budget_defaults(
    budget: &declorch_types::config::BudgetConfig,
...
async fn cron_deliver_response(
    kernel: &DeclorchKernel,
    agent_id: AgentId,
    response: &str,
    delivery: &declorch_types::scheduler::CronDelivery,
) -> Result<(), String> {
    use declorch_types::scheduler::CronDelivery;
    ...
    match delivery {
        ...
        CronDelivery::LastChannel => {
            #[cfg(feature = "memory")]
            let channel_info = kernel
                .memory
                .structured_get(agent_id, "delivery.last_channel");
            #[cfg(not(feature = "memory"))]
            let channel_info: Result<Option<serde_json::Value>, declorch_types::error::DeclorchError> =
                Ok(None);
...
struct KernelCronBridge {
    kernel: Arc<DeclorchKernel>,
}

#[async_trait]
impl declorch_channels::bridge::ChannelBridgeHandle for KernelCronBridge {
...
async fn cron_fan_out_targets(
    ...
    job_name: &str,
    output: &str,
    targets: &[declorch_types::scheduler::CronDeliveryTarget],
) {
    if targets.is_empty() || output.is_empty() {
        return;
    }
    let bridge: Arc<dyn declorch_channels::bridge::ChannelBridgeHandle> =
...
#[async_trait]
impl KernelHandle for DeclorchKernel {
    async fn spawn_agent(
        ...
        let content_hash = declorch_types::manifest_signing::hash_manifest(manifest_toml);
    ...
    fn kill_agent(&self, agent_id: &str) -> Result<(), String> {
        ...
        DeclorchKernel::kill_agent(self, id).map_err(|e| format!("Kill failed: {e}"))
    }
    ...
    fn activate_agent(&self, agent_id: &str) -> Result<String, String> {
        ...
        DeclorchKernel::activate_agent(self, id).map_err(|e| format!("Activate failed: {e}"))
    }
    ...
    fn memory_store(&self, key: &str, value: serde_json::Value) -> Result<(), String> {
        ...
        #[cfg(not(feature = "memory"))]
        Err("Memory is not available".to_string())
    }
    ...
    async fn task_post(
        ...
        #[cfg(not(feature = "memory"))]
        Err("Task system requires memory feature".to_string())
    }
    ...
    async fn publish_event(
        ...
        DeclorchKernel::publish_event(self, event).await;
        Ok(())
    }
    ...
    async fn knowledge_add_entity(
        ...
        #[cfg(not(feature = "memory"))]
        Err("Knowledge system requires memory feature".to_string())
    }
    ...
    async fn cron_create(
        ...
        let aid = declorch_types::agent::AgentId(
    ...
    async fn cron_cancel(&self, job_id: &str) -> Result<(), String> {
        let id = declorch_types::scheduler::CronJobId(
    ...
    async fn request_approval(
        ...
        use declorch_types::approval::{ApprovalDecision, ApprovalRequest as TypedRequest};
    ...
    async fn send_channel_message(
        ...
        let user = declorch_channels::types::ChannelUser {
        ...
        let formatted = if channel == "wecom" {
            ...
            declorch_channels::formatter::format_for_wecom(message, output_format)
        } else {
            message.to_string()
        };

        let content = declorch_channels::types::ChannelContent::Text(formatted);
    ...
    async fn send_channel_media(
        ...
        let content = match media_type {
            "image" => declorch_channels::types::ChannelContent::Image {
                url: media_url.to_string(),
                caption: caption.map(|s| s.to_string()),
            },
            "file" => declorch_channels::types::ChannelContent::File {
                url: media_url.to_string(),
    ...
    async fn send_channel_file_data(
        ...
        let content = declorch_channels::types::ChannelContent::FileData {
            data,
    ...
    async fn spawn_agent_checked(
        ...
        declorch_types::capability::validate_capability_inheritance(parent_caps, &child_caps)?;
...
#[async_trait]
impl declorch_wire::peer::PeerHandle for DeclorchKernel {
    fn local_agents(&self) -> Vec<declorch_wire::message::RemoteAgentInfo> {
        self.registry
            .list()
            .iter()
            .map(|entry| declorch_wire::message::RemoteAgentInfo {
    ...
    fn discover_agents(&self, query: &str) -> Vec<declorch_wire::message::RemoteAgentInfo> {
        let q = query.to_lowercase();
        self.registry
            .list()
            .iter()
...
mod tests {
    use super::*;
    use declorch_types::config::ExecPolicy;
    ...
    fn test_merge_preserves_workspace_when_disk_omits_it() {
        let entry = AgentManifest {
            ...
            autonomous: None,
            pinned_model: None,
            workspace: Some(std::path::PathBuf::from("/var/lib/declorch/agents/demo")),
            state_dir: None,
            generate_identity_files: true,
    ...
    fn test_exec_policy_reinherits_from_kernel_config_on_restart() {
        use declorch_types::config::ExecSecurityMode;
    ...
    fn test_manifest_to_capabilities_with_profile() {
        use declorch_types::agent::ToolProfile;
        let manifest = AgentManifest {
    ...
    fn test_hand_activation_does_not_seed_runtime_tool_filters() {
        ...
        let home_dir = tmp.path().join("declorch-kernel-hand-test");
        ...
        let kernel = DeclorchKernel::boot_with_config(config).expect("Kernel should boot");
        let instance = kernel
    ...
    fn test_hand_owned_agent_stop_clears_hand_for_reactivation() {
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-kernel-hand-stop-test");
        std::fs::create_dir_all(&home_dir).unwrap();
        ...
        let kernel = DeclorchKernel::boot_with_config(config).expect("kernel boots");
    ...
    fn test_activate_agent_wakes_suspended_and_crashed() {
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-kernel-activate-test");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_activate_agent_handle_accepts_name_and_uuid() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-kernel-activate-handle-test");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    /// surface.
    fn register_test_agent(kernel: &DeclorchKernel, name: &str) -> AgentId {
        let agent_id = AgentId::new();
        let entry = AgentEntry {
            id: agent_id,
            name: name.to_string(),
    ...
    fn test_migrate_shared_memory_schedules_imports_legacy_entries() {
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-migrate");
        std::fs::create_dir_all(&home_dir).unwrap();
        let config = KernelConfig {
            home_dir: home_dir.clone(),
            data_dir: home_dir.join("data"),
        ...
        kernel
            .memory
            .structured_set(shared, "__declorch_schedules", legacy_entries)
            .unwrap();
        ...
        let remaining = kernel
            .memory
            .structured_get(shared, "__declorch_schedules")
            .unwrap();
        assert_eq!(remaining, Some(serde_json::Value::Array(vec![])));
        let marker = kernel
            .memory
            .structured_get(shared, "__declorch_schedules_migrated_v1")
            .unwrap();
    ...
    fn test_migrate_shared_memory_schedules_is_idempotent() {
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-migrate-idem");
        std::fs::create_dir_all(&home_dir).unwrap();
        let config = KernelConfig {
            home_dir: home_dir.clone(),
            data_dir: home_dir.join("data"),
    ...
    fn test_migrate_shared_memory_schedules_skips_unknown_agent() {
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-migrate-skip");
        std::fs::create_dir_all(&home_dir).unwrap();
        let config = KernelConfig {
            home_dir: home_dir.clone(),
            data_dir: home_dir.join("data"),
    ...
    fn test_subprocess_timeout_hot_reload_fallback_providers() {
        ...
        use declorch_types::config::FallbackProviderConfig;

        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1129-fallback-timeout");
        std::fs::create_dir_all(&home_dir).unwrap();
        ...
        let kernel = DeclorchKernel::boot_with_config(config.clone()).expect("kernel boots");
    ...
    fn test_subprocess_timeout_hot_reload_default_model() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1129-default-timeout");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_subprocess_timeout_hot_reload_adds_new_fallback() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1129-add-fallback");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_referenced_providers_only_includes_configured_ones() {
        use declorch_types::config::{DefaultModelConfig, FallbackProviderConfig};

        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1031-referenced");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_1188_referenced_providers_resolves_alias_to_provider() {
        use declorch_types::config::DefaultModelConfig;

        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1188-alias");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_1188_referenced_providers_walks_channel_overrides() {
        use declorch_types::config::{
            ChannelOverrides, ChannelsConfig, DefaultModelConfig, TelegramConfig,
        };

        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1188-channel");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_1188_referenced_providers_walks_mcp_env() {
        use declorch_types::config::{DefaultModelConfig, McpServerConfigEntry, McpTransportEntry};

        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1188-mcp");
        std::fs::create_dir_all(&home_dir).unwrap();
    ...
    fn test_1188_referenced_providers_walks_skill_tags() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1188-skill");
        std::fs::create_dir_all(&home_dir).unwrap();
...
"#;
    ...
    // ----------------------------------------------------------------------
    // Issue #1140: agents placed at ~/.declorch/agents/<name>/agent.toml
    // must auto-spawn on boot so they appear in the chat tab.
    // ----------------------------------------------------------------------
    #[test]
    fn test_1140_auto_spawn_agents_from_disk() {
        let tmp = tempfile::tempdir().unwrap();
        let home_dir = tmp.path().join("declorch-1140");
...
description = "A user-installed agent placed in ~/.declorch/agents"
...
"#;
        ...
        let entry = kernel
            ...
            .expect("my-custom-agent must be auto-spawned from ~/.declorch/agents");
        ...
        let kernel2 = DeclorchKernel::boot_with_config(config2).expect("kernel re-boots");
    ...
    #[test]
    fn test_workspace_outside_declorch_stays_clean() {
