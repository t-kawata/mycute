...
#[cfg(feature = "memory")]
use declorch_memory::usage::UsageStore;
use declorch_types::usage::{ModelUsage, UsageRecord, UsageSummary};
use declorch_types::agent::{AgentId, ResourceQuota};
...
impl MeteringEngine {
    /// Create a new metering engine (with SQLite storage).
    #[cfg(feature = "memory")]
    pub fn new(store: Arc<UsageStore>) -> Self {
        Self { store }
    }

    /// Create a no-op metering engine (no SQLite storage).
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> DeclorchResult<()> {
        #[cfg(feature = "memory")]
        {
            // Hourly check
            if quota.max_cost_per_hour_usd > 0.0 {
                ...
                if hourly_cost >= quota.max_cost_per_hour_usd {
                    return Err(DeclorchError::QuotaExceeded(format!(
    ...
    pub fn budget_status(&self, budget: &declorch_types::config::BudgetConfig) -> BudgetStatus {
        #[cfg(feature = "memory")]
        let (hourly, daily, monthly) = {
            let h = self.store.query_global_hourly().unwrap_or(0.0);
            let d = self.store.query_today_cost().unwrap_or(0.0);
            let m = self.store.query_global_monthly().unwrap_or(0.0);
            (h, d, m)
        };
        #[cfg(not(feature = "memory"))]
        let (hourly, daily, monthly) = (0.0, 0.0, 0.0);
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> DeclorchResult<UsageSummary> {
        #[cfg(feature = "memory")]
    ...
    pub fn get_by_model(&self) -> DeclorchResult<Vec<ModelUsage>> {
        #[cfg(feature = "memory")]
    ...
    pub fn estimate_cost_with_catalog(
        catalog: &declorch_runtime::model_catalog::ModelCatalog,
        model: &str,
        input_tokens: u64,
        output_tokens: u64,
    ) -> f64 {
    ...
    pub fn cleanup(&self, days: u32) -> DeclorchResult<usize> {
        #[cfg(feature = "memory")]
...
mod tests {
    ...
    fn test_estimate_cost_with_catalog() {
        let catalog = declorch_runtime::model_catalog::ModelCatalog::new();
