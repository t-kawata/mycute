//! Configuration loading from `~/.declorch/config.toml` with defaults.
//!
...
use declorch_types::config::KernelConfig;
...
fn lenient_extract_bindings(root_value: &mut toml::Value) {
    use declorch_types::config::AgentBinding;
...
///
/// Respects `OPENFANG_HOME` env var (e.g. `OPENFANG_HOME=/opt/declorch`).
pub fn default_config_path() -> PathBuf {
    declorch_home().join("config.toml")
}

/// Get the Declorch home directory.
///
/// Priority: `OPENFANG_HOME` env var > `~/.declorch`.
pub fn declorch_home() -> PathBuf {
