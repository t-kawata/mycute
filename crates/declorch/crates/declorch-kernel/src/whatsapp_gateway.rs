...
//!
//! Embeds the gateway JS at compile time, extracts it to `~/.declorch/whatsapp-gateway/`,
//! runs `npm install` if needed, and spawns `node index.js` as a managed child process
//! that auto-restarts on crash.

use crate::config::declorch_home;
...
fn gateway_dir() -> PathBuf {
    declorch_home().join("whatsapp-gateway")
}
...
pub async fn start_whatsapp_gateway(kernel: &Arc<super::kernel::DeclorchKernel>) {
    ...
    let declorch_url = format!("http://{api_listen}");
...
mod tests {
    ...
    fn test_embedded_files_not_empty() {
        ...
        assert!(GATEWAY_PACKAGE_JSON.contains("@declorch/whatsapp-gateway"));
    }
    ...
    #[test]
    fn test_gateway_dir_under_declorch_home() {
        let dir = gateway_dir();
    ...
    fn test_write_if_changed_creates_new_file() {
        let tmp = std::env::temp_dir().join("declorch_test_gateway");
