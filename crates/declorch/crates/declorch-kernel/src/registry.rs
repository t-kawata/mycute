...
use dashmap::DashMap;
use declorch_types::agent::{AgentEntry, AgentId, AgentMode, AgentState};
...
impl AgentRegistry {
    ...
    pub fn register(&self, entry: AgentEntry) -> DeclorchResult<()> {
        if self.name_index.contains_key(&entry.name) {
            return Err(DeclorchError::AgentAlreadyExists(entry.name.clone()));
        }
        let id = entry.id;
    ...
    /// Update agent state.
    pub fn set_state(&self, id: AgentId, state: AgentState) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
        entry.state = state;
    ...
    pub fn set_mode(&self, id: AgentId, mode: AgentMode) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn remove(&self, id: AgentId) -> DeclorchResult<AgentEntry> {
        let (_, entry) = self
            .agents
            .remove(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_session_id(
        &self,
        id: AgentId,
        new_session_id: declorch_types::agent::SessionId,
    ) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_identity(
        &self,
        id: AgentId,
        identity: declorch_types::agent::AgentIdentity,
    ) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_model(&self, id: AgentId, new_model: String) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_fallback_models(
        &self,
        id: AgentId,
        fallback_models: Vec<declorch_types::agent::FallbackModel>,
    ) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
    ...
    pub fn update_skills(&self, id: AgentId, skills: Vec<String>) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_mcp_servers(&self, id: AgentId, servers: Vec<String>) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_system_prompt(&self, id: AgentId, new_prompt: String) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn update_name(&self, id: AgentId, new_name: String) -> DeclorchResult<()> {
        if let Some(existing_id) = self.name_index.get(&new_name).as_deref().copied() {
            if existing_id != id {
                return Err(DeclorchError::AgentAlreadyExists(new_name));
            }
    ...
    pub fn update_description(&self, id: AgentId, new_desc: String) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
    ...
    pub fn mark_onboarding_complete(&self, id: AgentId) -> DeclorchResult<()> {
        let mut entry = self
            .agents
            .get_mut(&id)
            .ok_or_else(|| DeclorchError::AgentNotFound(id.to_string()))?;
