...
//! to Declorch users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use declorch_types::agent::UserId;
use declorch_types::config::UserConfig;
...
pub struct AuthManager {
    /// Known users by their Declorch user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Declorch UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> DeclorchResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| DeclorchError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(DeclorchError::AuthDenied(format!(
