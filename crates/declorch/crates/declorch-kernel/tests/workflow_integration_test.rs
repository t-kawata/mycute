...
async fn test_trigger_registration_with_kernel() {
    use declorch_kernel::triggers::TriggerPattern;
...
async fn test_workflow_e2e_with_groq() {
    ...
    assert!(matches!(
        run.state,
        declorch_kernel::workflow::WorkflowRunState::Completed
    ));
