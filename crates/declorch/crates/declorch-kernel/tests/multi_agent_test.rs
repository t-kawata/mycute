...
//!
//! Run with: GROQ_API_KEY=gsk_... cargo test -p declorch-kernel --test multi_agent_test -- --nocapture
...
fn test_config() -> KernelConfig {
    let tmp = std::env::temp_dir().join("declorch-multi-agent-test");
...
async fn test_six_agent_fleet() {
    ...
    let kernel = DeclorchKernel::boot_with_config(test_config()).expect("Kernel should boot");
