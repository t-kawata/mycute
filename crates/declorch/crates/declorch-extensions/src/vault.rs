...
//!
//! Stores secrets in `~/.declorch/vault.enc`, with the master key sourced from
...
#[cfg(not(test))]
const KEYRING_SERVICE: &str = "declorch-vault";
...
impl CredentialVault {
    ...
    pub fn unlock(&mut self) -> ExtensionResult<()> {
        ...
        if !self.path.exists() {
            return Err(ExtensionError::Vault(
                "Vault not initialized. Run `declorch vault init`.".to_string(),
            ));
        }
...
fn machine_fingerprint() -> Vec<u8> {
    ...
    hasher.update(b"declorch-vault-v1");
