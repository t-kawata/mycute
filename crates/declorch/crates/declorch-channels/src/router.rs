...
use declorch_types::config::{AgentBinding, BroadcastConfig, BroadcastStrategy};
use std::sync::Mutex;
use tracing::warn;
...
pub struct AgentRouter {
    /// Default agent per user (keyed by declorch_user or platform_id).
...
mod tests {
    ...
    fn test_binding_channel_match() {
        ...
        router.load_bindings(&[AgentBinding {
            ...
            match_rule: declorch_types::config::BindingMatchRule {
    ...
    fn test_binding_specificity_scores() {
        use declorch_types::config::BindingMatchRule;
    ...
    fn test_binding_match_rule_unknown_field_rejected() {
        ...
        let r: Result<declorch_types::config::BindingMatchRule, _> = serde_json::from_str(bad);
        ...
        let r: declorch_types::config::BindingMatchRule = serde_json::from_str(good).unwrap();
