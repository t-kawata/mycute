...
pub struct ChannelUser {
    ...
    /// Optional mapping to an Declorch user identity.
    pub declorch_user: Option<String>,
}
...
// Re-export the adapter allowlist from declorch-types so config validation
// and routing share a single source of truth (no drift between the two).
pub use declorch_types::config::CHANNELS_WITH_PLATFORM_ID_AS_CHANNEL;
...
// Re-export policy/format types from declorch-types for convenience.
pub use declorch_types::config::{DmPolicy, GroupPolicy, OutputFormat};

/// Trait that every channel adapter must implement.
///
/// A channel adapter bridges a messaging platform to the Declorch kernel by converting
...
pub fn split_message(text: &str, max_len: usize) -> Vec<&str> {
    ...
    while !remaining.is_empty() {
        ...
        let safe_end = declorch_types::truncate_str(remaining, max_len).len();
