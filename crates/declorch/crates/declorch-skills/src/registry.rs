...
pub struct SkillRegistry {
    ...
    /// `~/.declorch/config.toml`). Used by the loader to resolve declared
                ...
                if openclaw_compat::detect_skillmd(&path) {
                    match openclaw_compat::convert_skillmd(&path) {
                        Ok(converted) => {
                            ...
                            info!(
                                skill = %converted.manifest.skill.name,
                                "Auto-converting SKILL.md to Declorch format"
                            );
                            if let Err(e) =
                                openclaw_compat::write_declorch_manifest(&path, &converted.manifest)
                            {
