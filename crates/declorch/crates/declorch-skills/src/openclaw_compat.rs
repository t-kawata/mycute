...
//!
//! This module detects both formats and converts them to Declorch `SkillManifest`.
...
use declorch_types::tool_compat;
...
/// Result of converting a SKILL.md into Declorch format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → declorch_name).
...
/// Full conversion of a SKILL.md directory to Declorch format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to Declorch tool definitions
        for cmd in &meta.commands {
            ...
            let declorch_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_declorch_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into Declorch format.
///
...
/// Convert an OpenClaw Node.js skill directory into an Declorch SkillManifest.
///
...
/// Write an Declorch skill.toml manifest for an OpenClaw skill.
pub fn write_declorch_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
