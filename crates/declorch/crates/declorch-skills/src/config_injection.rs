...
//!
//! 1. User-supplied config (e.g., `[skills.<skill-name>]` in `~/.declorch/config.toml`)
...
///    of `~/.declorch/config.toml`, passed in by the caller.
...
pub fn render_config_block(resolved: &HashMap<String, String>) -> String {
    ...
    let mut out = String::from("[Skill config from ~/.declorch/config.toml:\n");
    for key in keys {
