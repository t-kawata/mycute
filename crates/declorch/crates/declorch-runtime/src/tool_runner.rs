...
use declorch_skills::registry::SkillRegistry;
use declorch_types::taint::{TaintLabel, TaintSink, TaintedValue};
use declorch_types::tool::{ToolDefinition, ToolResult};
use declorch_types::tool_compat::normalize_tool_name;
...
pub async fn execute_tool(
    ...
    exec_policy: Option<&declorch_types::config::ExecPolicy>,
    tts_engine: Option<&crate::tts::TtsEngine>,
    docker_config: Option<&declorch_types::config::DockerSandboxConfig>,
    process_manager: Option<&crate::process_manager::ProcessManager>,
) -> ToolResult {
    // Normalize the tool name through compat mappings so LLM-hallucinated aliases
    // (e.g. "fs-write" → "file_write") resolve to the canonical Declorch name.
    ...
    let exec_policy_bypasses_approval = is_shell_tool(tool_name)
        && exec_policy.is_some_and(|p| {
            p.mode == declorch_types::config::ExecSecurityMode::Full
                || (p.mode == declorch_types::config::ExecSecurityMode::Allowlist
    ...
    if let Some(kh) = kernel {
        if !exec_policy_bypasses_approval && kh.requires_approval(tool_name) {
            ...
            let summary = format!(
                "{}: {}",
                tool_name,
                declorch_types::truncate_str(&input_str, 200)
            );
    ...
    let result = match tool_name {
        ...
        "shell_exec" => {
            ...
            let is_full_exec = exec_policy
                .is_some_and(|p| p.mode == declorch_types::config::ExecSecurityMode::Full);
            if !is_full_exec {
        ...
        other => {
            ...
            // Fallback 2: Skill registry tool providers
                ...
                if let Some(skill) = registry.find_tool_provider(other) {
                    ...
                    match declorch_skills::loader::execute_skill_tool(
                        &skill.manifest,
                        &skill.path,
                        other,
                        input,
    ...
    vec![
        ...
        ToolDefinition {
            ...
            input_schema: serde_json::json!({
                "type": "object",
                "properties": {
                    "url": { "type": "string", "description": "Base URL of the remote Declorch/A2A-compatible agent (e.g., 'https://agent.example.com')" }
                },
                "required": ["url"]
            }),
        },
        ...
        // touching the filesystem. Global skills live at ~/.declorch/skills/
...
async fn tool_web_search_legacy(input: &serde_json::Value) -> Result<String, String> {
    ...
    let resp = client
        ...
        .query(&[("q", query)])
        .header("User-Agent", "Mozilla/5.0 (compatible; DeclorchAgent/0.1)")
        .send()
        .await
...
async fn tool_shell_exec(
    ...
    let use_direct_exec = exec_policy
        .map(|p| p.mode == declorch_types::config::ExecSecurityMode::Allowlist)
...
fn parse_entity_type(s: &str) -> declorch_types::memory::EntityType {
    use declorch_types::memory::EntityType;
...
fn parse_relation_type(s: &str) -> declorch_types::memory::RelationType {
    use declorch_types::memory::RelationType;
    match s.to_lowercase().as_str() {
...
async fn tool_knowledge_add_entity(
    ...
    let entity = declorch_types::memory::Entity {
...
async fn tool_knowledge_add_relation(
    ...
    let relation = declorch_types::memory::Relation {
...
async fn tool_knowledge_query(
    ...
    let pattern = declorch_types::memory::GraphPattern {
        source,
        relation,
        target,
        max_depth,
...
async fn tool_location_get() -> Result<String, String> {
    ...
    let resp = client
        ...
        .header("User-Agent", "Declorch/0.1")
        .send()
        .await
...
async fn tool_media_describe(
    ...
    let attachment = declorch_types::media::MediaAttachment {
        media_type: declorch_types::media::MediaType::Image,
        mime_type: mime.to_string(),
        source: declorch_types::media::MediaSource::Base64 {
...
async fn tool_media_transcribe(
    ...
    let attachment = declorch_types::media::MediaAttachment {
        media_type: declorch_types::media::MediaType::Audio,
        mime_type: mime.to_string(),
        source: declorch_types::media::MediaSource::Base64 {
            data: base64::engine::general_purpose::STANDARD.encode(&data),
            mime_type: mime.to_string(),
...
async fn tool_image_generate(
    ...
    let model = match model_str {
        "dall-e-3" | "dalle3" | "dalle-3" => declorch_types::media::ImageGenModel::DallE3,
        "dall-e-2" | "dalle2" | "dalle-2" => declorch_types::media::ImageGenModel::DallE2,
        "gpt-image-1" | "gpt_image_1" => declorch_types::media::ImageGenModel::GptImage1,
        _ => {
            return Err(format!(
    ...
    let request = declorch_types::media::ImageGenRequest {
    ...
    {
        use base64::Engine;
        let upload_dir = std::env::temp_dir().join("declorch_uploads");
...
async fn tool_speech_to_text(
    ...
    use declorch_types::media::{MediaAttachment, MediaSource, MediaType};
...
// Skill introspection tools (issue #1038)
//
// Global skills live at ~/.declorch/skills/ which is outside the agent
...
fn tool_skill_list(skill_registry: Option<&SkillRegistry>) -> Result<String, String> {
    ...
    if skills.is_empty() {
        return Ok("No skills installed. Install skills via the dashboard or `declorch skill install <name>`.".to_string());
    }
...
mod tests {
    ...
    async fn test_file_read_missing() {
        let bad_path = std::env::temp_dir()
            .join("declorch_test_nonexistent_99999")
            .join("file.txt");
    ...
    async fn test_capability_enforcement_allowed() {
        ...
        let bad_path = std::env::temp_dir()
            .join("declorch_test_nonexistent_12345")
            .join("file.txt");
        let result = execute_tool(
            "test-id",
            "file_read",
    ...
    async fn test_canvas_present_tool() {
        ...
        let tmp = std::env::temp_dir().join("declorch_canvas_test");
    ...
    async fn test_issue_919_process_start_rm_blocked_in_allowlist() {
        use declorch_types::config::{ExecPolicy, ExecSecurityMode};
        ...
        let input = serde_json::json!({
            "command": "rm",
            "args": ["/tmp/declorch_test_should_not_be_deleted.txt"],
        });
    ...
    // the kernel, without booting a real DeclorchKernel.
    ...
    impl crate::kernel_handle::KernelHandle for FakeKernelHandle {
        ...
        async fn knowledge_add_entity(
            &self,
            _entity: declorch_types::memory::Entity,
        ) -> Result<String, String> {
            Err("not used".into())
        }
        async fn knowledge_add_relation(
            &self,
            _relation: declorch_types::memory::Relation,
        ) -> Result<String, String> {
            Err("not used".into())
        }
        async fn knowledge_query(
            &self,
            _pattern: declorch_types::memory::GraphPattern,
        ...
        async fn cron_create(
            ...
            // declorch-kernel): `{ "job_id": "...", "status": "created" }`.
