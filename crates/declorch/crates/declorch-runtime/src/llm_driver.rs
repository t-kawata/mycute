...
use declorch_types::message::{ContentBlock, Message, StopReason, TokenUsage};
...
pub struct CompletionRequest {
    ...
    pub thinking: Option<declorch_types::config::ThinkingConfig>,
}
...
pub struct DriverConfig {
    ...
    /// `claude` CLI.  Defaults to `true` because Declorch runs as a daemon
    /// with no interactive terminal, so permission prompts would block
    /// indefinitely.  Declorch's own capability / RBAC layer already
