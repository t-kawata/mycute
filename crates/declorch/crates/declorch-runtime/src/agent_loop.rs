...
use declorch_types::session::Session;
#[cfg(feature = "memory")]
use declorch_memory::MemorySubstrate;
use declorch_skills::registry::SkillRegistry;
use declorch_types::agent::{AgentManifest, FallbackModel};
use declorch_types::error::{DeclorchError, DeclorchResult};
#[cfg(feature = "memory")]
use declorch_types::memory::{Memory, MemoryFilter, MemorySource};
use declorch_types::message::{
    ContentBlock, Message, MessageContent, Role, StopReason, TokenUsage,
};
use declorch_types::tool::{ToolCall, ToolDefinition};
#[cfg(feature = "memory")]
...
#[allow(dead_code)]
const MAX_HISTORY_MESSAGES: usize = declorch_types::agent::DEFAULT_MAX_HISTORY_MESSAGES;
...
pub struct AgentLoopResult {
    ...
    pub directives: declorch_types::message::ReplyDirectives,
}
...
///
/// This is the core of Declorch: it loads session context, recalls memories,
...
pub async fn run_agent_loop(
    ...
    user_message: &str,
    session: &mut Session,
    #[cfg(feature = "memory")] memory: &MemorySubstrate,
...
) -> DeclorchResult<AgentLoopResult> {
    ...
    let memories: Vec<declorch_types::memory::MemoryFragment> = Vec::new();
    ...
    if let Some(hook_reg) = hooks {
        let ctx = crate::hooks::HookContext {
            ...
            event: declorch_types::agent::HookEvent::BeforePromptBuild,
            ...
            StopReason::EndTurn | StopReason::StopSequence => {
                ...
                if is_silent_token(&text) || parsed_directives.silent {
                    ...
                    return Ok(AgentLoopResult {
                        ...
                        cost_usd: None,
                        silent: true,
                        directives: declorch_types::message::ReplyDirectives {
                ...
                if let Some(hook_reg) = hooks {
                    let ctx = crate::hooks::HookContext {
                        agent_name: &manifest.name,
                        agent_id: agent_id_str.as_str(),
                        event: declorch_types::agent::HookEvent::AgentLoopEnd,
                        data: serde_json::json!({
                            "iterations": iteration + 1,
                ...
                for tool_call in deduplicate_tool_calls(&response) {
                    ...
                    match &verdict {
                        LoopGuardVerdict::CircuitBreak(msg) => {
                            ...
                            return Err(DeclorchError::Internal(msg.clone()));
                        }
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: declorch_types::agent::HookEvent::BeforeToolCall,
                            data: serde_json::json!({
                        ...
                        Some(timeout) => {
                            ...
                            match tokio::time::timeout(timeout, exec_fut).await {
                                ...
                                Err(_) => {
                                    ...
                                    declorch_types::tool::ToolResult {
                                        tool_use_id: tool_call.id.clone(),
                                        content: format!(
                    ...
                    if let Some(hook_reg) = hooks {
                        let ctx = crate::hooks::HookContext {
                            ...
                            event: declorch_types::agent::HookEvent::AfterToolCall,
                            data: serde_json::json!({
                                "tool_name": &tool_call.name,
    ...
    Err(DeclorchError::MaxIterationsExceeded(max_iterations))
}
...
) -> DeclorchResult<crate::llm_driver::CompletionResponse> {
    ...
    if let (Some(provider), Some(cooldown)) = (provider, cooldown) {
        match cooldown.check(provider) {
            CooldownVerdict::Reject {
                ...
                return Err(DeclorchError::LlmDriver(format!(
    ...
    for attempt in 0..=MAX_RETRIES {
        match driver.complete(request.clone()).await {
            ...
            Err(e) => {
                ...
                return Err(DeclorchError::LlmDriver(user_msg));
            }
        }
    }

    Err(DeclorchError::LlmDriver(
...
mod tests {
    ...
    use declorch_types::tool::ToolCall;
    ...
    fn test_max_history_messages() {
        ...
        assert_eq!(
            declorch_types::agent::DEFAULT_MAX_HISTORY_MESSAGES,
    ...
    fn test_manifest_max_history_round_trip_json() {
        ...
        let manifest: declorch_types::agent::AgentManifest =
    ...
    async fn test_empty_response_after_tool_use_returns_fallback() {
        let memory = declorch_memory::MemorySubstrate::open_in_memory(0.01).unwrap();
        let agent_id = declorch_types::agent::AgentId::new();
        let mut session = declorch_types::session::Session {
            id: declorch_types::agent::SessionId::new(),
            agent_id,
            messages: Vec::new(),
