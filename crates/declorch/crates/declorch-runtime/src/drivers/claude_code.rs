...
use dashmap::DashMap;
use declorch_types::message::{ContentBlock, MessageContent, Role, StopReason, TokenUsage};
        ...
        if skip_permissions {
            warn!(
                "Claude Code driver: --dangerously-skip-permissions enabled. \
                 ...
                 Declorch's own capability/RBAC system enforces access control."
            );
        }
...
impl LlmDriver for ClaudeCodeDriver {
    async fn complete(&self, request: CompletionRequest) -> Result<CompletionResponse, LlmError> {
        ...
        // Declorch runs as a service without a login shell.
...
mod tests {
    ...
    fn test_build_prompt_simple() {
        use declorch_types::message::{Message, MessageContent};
    ...
    fn test_build_prompt_renders_image_attachment_marker() {
        use declorch_types::message::{ContentBlock, Message, MessageContent};
