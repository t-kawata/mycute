...
/// File name for persisted OAuth tokens (inside ~/.declorch/).
...
impl PersistedTokens {
    ...
    /// Load from the Declorch data directory.
    pub fn load(declorch_dir: &Path) -> Option<Self> {
        let path = declorch_dir.join(TOKEN_FILE_NAME);
        let data = std::fs::read_to_string(&path).ok()?;
        serde_json::from_str(&data).ok()
    }

    /// Persist to the Declorch data directory with restricted permissions.
    pub fn save(&self, declorch_dir: &Path) -> Result<(), String> {
        let path = declorch_dir.join(TOKEN_FILE_NAME);
...
pub async fn exchange_copilot_token(
    ...
    let resp = client
        ...
        .header("Accept", "application/json")
        .header("User-Agent", "Declorch/1.0")
...
impl CopilotDriver {
    pub fn new(declorch_dir: PathBuf) -> Self {
        ...
        let persisted = PersistedTokens::load(&declorch_dir);
    ...
    async fn ensure_access_token(&self) -> Result<String, crate::llm_driver::LlmError> {
        ...
        if let Some(ref rt) = refresh_token {
            match refresh_access_token(&self.http_client, rt).await {
                Ok(new_tokens) => {
                    ...
                    if let Err(e) = new_tokens.save(&self.declorch_dir) {
        ...
        Err(crate::llm_driver::LlmError::AuthenticationFailed(
            "Copilot OAuth tokens expired. Run `declorch config set-key github-copilot` to re-authenticate via device flow.".to_string(),
        ))
    }
...
///
/// Called from `declorch config set-key github-copilot`, `declorch init`,
/// `declorch onboard`, and `declorch configure`.
pub async fn run_interactive_setup(declorch_dir: &Path) -> Result<PersistedTokens, String> {
    run_device_flow(declorch_dir).await
}
...
pub async fn run_device_flow(declorch_dir: &Path) -> Result<PersistedTokens, String> {
...
pub fn copilot_auth_available(declorch_dir: &Path) -> bool {
    declorch_dir.join(TOKEN_FILE_NAME).exists()
}
