...
//! - `build_agent_card` — expose Declorch agents via A2A
//! - `A2aClient` — discover and interact with external A2A agents

use declorch_types::agent::AgentManifest;
...
pub struct AgentCard {
    ...
    /// Skills this agent can perform (A2A skill descriptors, not Declorch skills).
...
/// A2A skill descriptor (not an Declorch skill — describes a capability).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
pub async fn discover_external_agents(
    agents: &[declorch_types::config::ExternalAgent],
...
// ---------------------------------------------------------------------------
// A2A Server — expose Declorch agents via A2A
// ---------------------------------------------------------------------------

/// Build an A2A Agent Card from an Declorch agent manifest.
...
impl A2aClient {
    ...
    pub async fn discover(&self, url: &str) -> Result<AgentCard, String> {
        ...
        let response = self
            .client
            .get(&agent_json_url)
            .header("User-Agent", "Declorch/0.1 A2A")
            .send()
            .await
...
mod tests {
    ...
    fn test_a2a_config_serde() {
        use declorch_types::config::{A2aConfig, ExternalAgent};
