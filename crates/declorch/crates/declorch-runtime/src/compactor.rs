...
pub fn estimate_token_count(
    ...
    tools: Option<&[declorch_types::tool::ToolDefinition]>,
) -> usize {
...
fn adjust_split_for_tool_pairs(messages: &[Message], split: usize) -> usize {
    use declorch_types::message::{ContentBlock, Role};
...
mod tests {
    use super::*;
    use declorch_types::message::TokenUsage;

    #[test]
    fn test_needs_compaction_below_threshold() {
        let session = Session {
            id: declorch_types::agent::SessionId::new(),
            agent_id: declorch_types::agent::AgentId::new(),
