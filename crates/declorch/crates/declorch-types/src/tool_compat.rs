//! Shared tool name mappings between OpenClaw and Declorch.
//!
//! These mappings are used by both the migration engine and the skill system
//! to normalize OpenClaw tool names into Declorch equivalents.

/// Map an OpenClaw tool name to its Declorch equivalent.
///
/// Returns `None` if the name has no known mapping (may already be
/// an Declorch tool name — check with [`is_known_declorch_tool`]).
...
/// Normalize a tool name to its canonical Declorch form.
///
/// If the name is already a known Declorch tool, returns it as-is.
...
pub fn normalize_tool_name(name: &str) -> &str {
    if is_known_declorch_tool(name) {
        return name;
    }
    map_tool_name(name).unwrap_or(name)
}

/// Check if a tool name is a known Declorch built-in tool.
pub fn is_known_declorch_tool(name: &str) -> bool {
    matches!(
        name,
        "file_read"
            | "file_write"
...
mod tests {
    ...
    fn test_normalize_tool_name() {
        // Known Declorch tools pass through unchanged
    ...
    #[test]
    fn test_is_known_declorch_tool() {
        ...
        for tool in &known {
            assert!(is_known_declorch_tool(tool), "Expected {tool} to be known");
        }

        // Unknown
        assert!(!is_known_declorch_tool("unknown"));
        assert!(!is_known_declorch_tool("Read"));
        assert!(!is_known_declorch_tool("Bash"));
    }
}
