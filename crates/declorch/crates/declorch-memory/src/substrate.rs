...
use declorch_types::agent::{AgentEntry, AgentId, SessionId};
use declorch_types::config::MemoryConfig;
use declorch_types::error::{DeclorchError, DeclorchResult};
use declorch_types::memory::{
...
impl MemorySubstrate {
    ...
    pub fn open(
        ...
        let conn = Connection::open(db_path).map_err(|e| DeclorchError::Memory(e.to_string()))?;
        conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000;")
            .map_err(|e| DeclorchError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| DeclorchError::Memory(e.to_string()))?;
    ...
    pub fn open_in_memory(decay_rate: f32) -> DeclorchResult<Self> {
        let conn =
            Connection::open_in_memory().map_err(|e| DeclorchError::Memory(e.to_string()))?;
        run_migrations(&conn).map_err(|e| DeclorchError::Memory(e.to_string()))?;
        let shared = Arc::new(Mutex::new(conn));
    ...
    pub fn save_agent(&self, entry: &AgentEntry) -> DeclorchResult<()> {
        self.structured.save_agent(entry)
    }

    /// Load an agent entry from persistent storage.
    pub fn load_agent(&self, agent_id: AgentId) -> DeclorchResult<Option<AgentEntry>> {
        self.structured.load_agent(agent_id)
    }

    /// Remove an agent from persistent storage and cascade-delete sessions.
    pub fn remove_agent(&self, agent_id: AgentId) -> DeclorchResult<()> {
    ...
    pub fn load_all_agents(&self) -> DeclorchResult<Vec<AgentEntry>> {
        self.structured.load_all_agents()
    }

    /// List all saved agents.
    pub fn list_agents(&self) -> DeclorchResult<Vec<(String, String, String)>> {
    ...
    pub fn structured_get(
        ...
        agent_id: AgentId,
        key: &str,
    ) -> DeclorchResult<Option<serde_json::Value>> {
        self.structured.get(agent_id, key)
    }

    /// List all KV pairs for an agent.
    pub fn list_kv(&self, agent_id: AgentId) -> DeclorchResult<Vec<(String, serde_json::Value)>> {
        self.structured.list_kv(agent_id)
    }

    /// Delete a KV entry for an agent.
    pub fn structured_delete(&self, agent_id: AgentId, key: &str) -> DeclorchResult<()> {
    ...
    /// Get a session by ID.
    pub fn get_session(&self, session_id: SessionId) -> DeclorchResult<Option<Session>> {
        self.sessions.get_session(session_id)
    }

    /// Save a session.
    pub fn save_session(&self, session: &Session) -> DeclorchResult<()> {
        self.sessions.save_session(session)
    }

    /// Save a session asynchronously — runs the SQLite write in a blocking
    /// thread so the tokio runtime stays responsive.
    pub async fn save_session_async(&self, session: &Session) -> DeclorchResult<()> {
        let sessions = self.sessions.clone();
        let session = session.clone();
        tokio::task::spawn_blocking(move || sessions.save_session(&session))
            .await
            .map_err(|e| DeclorchError::Internal(e.to_string()))?
    }

    /// Create a new empty session for an agent.
    pub fn create_session(&self, agent_id: AgentId) -> DeclorchResult<Session> {
        self.sessions.create_session(agent_id)
    }

    /// List all sessions with metadata.
    pub fn list_sessions(&self) -> DeclorchResult<Vec<serde_json::Value>> {
        self.sessions.list_sessions()
    }

    /// Delete a session by ID.
    pub fn delete_session(&self, session_id: SessionId) -> DeclorchResult<()> {
        self.sessions.delete_session(session_id)
    }

    /// Delete all sessions belonging to an agent.
    pub fn delete_agent_sessions(&self, agent_id: AgentId) -> DeclorchResult<()> {
        self.sessions.delete_agent_sessions(agent_id)
    }

    /// Delete the canonical (cross-channel) session for an agent.
    pub fn delete_canonical_session(&self, agent_id: AgentId) -> DeclorchResult<()> {
    ...
    pub fn find_session_by_label(
        ...
        agent_id: AgentId,
        label: &str,
    ) -> DeclorchResult<Option<Session>> {
        self.sessions.find_session_by_label(agent_id, label)
    }

    /// List all sessions for a specific agent.
    pub fn list_agent_sessions(&self, agent_id: AgentId) -> DeclorchResult<Vec<serde_json::Value>> {
    ...
    ) -> DeclorchResult<(Option<String>, Vec<declorch_types::message::Message>)> {
    ...
    pub fn store_llm_summary(
        ...
        agent_id: AgentId,
        summary: &str,
        kept_messages: Vec<declorch_types::message::Message>,
    ) -> DeclorchResult<()> {
        self.sessions
    ...
    pub fn append_canonical(
        &self,
        agent_id: AgentId,
        messages: &[declorch_types::message::Message],
    ...
    pub fn load_paired_devices(&self) -> DeclorchResult<Vec<serde_json::Value>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Memory(e.to_string()))?;
        ...
        for row in rows {
            devices.push(row.map_err(|e| DeclorchError::Memory(e.to_string()))?);
        }
        Ok(devices)
    }
    ...
    pub fn remove_paired_device(&self, device_id: &str) -> DeclorchResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Memory(e.to_string()))?;
    ...
    ) -> DeclorchResult<Vec<MemoryFragment>> {
        self.semantic
            .recall_with_embedding(query, limit, filter, query_embedding)
    }

    /// Update the embedding for an existing memory.
    pub fn update_embedding(&self, id: MemoryId, embedding: &[f32]) -> DeclorchResult<()> {
    ...
    pub async fn task_post(
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let db = conn.lock().map_err(|e| DeclorchError::Internal(e.to_string()))?;
            db.execute(
    ...
    pub async fn task_claim(&self, agent_id: &str) -> DeclorchResult<Option<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            match result {
                ...
                Err(e) => Err(DeclorchError::Memory(e.to_string())),
            }
        })
        .await
        .map_err(|e| DeclorchError::Internal(e.to_string()))?
    ...
    pub async fn task_complete(&self, task_id: &str, result: &str) -> DeclorchResult<()> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if rows == 0 {
                return Err(DeclorchError::Internal(format!("Task not found: {task_id}")));
            }
            Ok(())
        })
        .await
    ...
    pub async fn task_list(&self, status: Option<&str>) -> DeclorchResult<Vec<serde_json::Value>> {
        let conn = Arc::clone(&self.conn);
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let mut stmt = db.prepare(sql).map_err(|e| DeclorchError::Memory(e.to_string()))?;
            ...
            for row in rows {
                tasks.push(row.map_err(|e| DeclorchError::Memory(e.to_string()))?);
            }
            Ok(tasks)
        })
        .await
...
impl Memory for MemorySubstrate {
    async fn get(&self, agent_id: AgentId, key: &str) -> DeclorchResult<Option<serde_json::Value>> {
    ...
    async fn delete(&self, agent_id: AgentId, key: &str) -> DeclorchResult<()> {
        let store = self.structured.clone();
        let key = key.to_string();
    ...
    async fn forget(&self, id: MemoryId) -> DeclorchResult<()> {
        let store = self.semantic.clone();
    ...
    async fn add_entity(&self, entity: Entity) -> DeclorchResult<String> {
    ...
    async fn add_relation(&self, relation: Relation) -> DeclorchResult<String> {
        let store = self.knowledge.clone();
    ...
    async fn query_graph(&self, pattern: GraphPattern) -> DeclorchResult<Vec<GraphMatch>> {
        let store = self.knowledge.clone();
    ...
    async fn consolidate(&self) -> DeclorchResult<ConsolidationReport> {
    ...
    async fn export(&self, format: ExportFormat) -> DeclorchResult<Vec<u8>> {
        let _ = format;
        Ok(Vec::new())
    }

    async fn import(&self, _data: &[u8], _format: ExportFormat) -> DeclorchResult<ImportReport> {
        Ok(ImportReport {
            entities_imported: 0,
