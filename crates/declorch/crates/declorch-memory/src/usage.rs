...
//!
//! The usage data types are defined in `declorch-types` to avoid a dependency
//! on rusqlite for crates that only need the data structures.

pub use declorch_types::usage::{DailyBreakdown, ModelUsage, UsageRecord, UsageSummary};
...
impl UsageStore {
    ...
    pub fn record(&self, record: &UsageRecord) -> DeclorchResult<()> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn query_hourly(&self, agent_id: AgentId) -> DeclorchResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_daily(&self, agent_id: AgentId) -> DeclorchResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_monthly(&self, agent_id: AgentId) -> DeclorchResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_global_hourly(&self) -> DeclorchResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_global_monthly(&self) -> DeclorchResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_summary(&self, agent_id: Option<AgentId>) -> DeclorchResult<UsageSummary> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_by_model(&self) -> DeclorchResult<Vec<ModelUsage>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
        ...
        for row in rows {
            results.push(row.map_err(|e| DeclorchError::Memory(e.to_string()))?);
        }
        Ok(results)
    }

    /// Query daily usage breakdown for the last N days.
    pub fn query_daily_breakdown(&self, days: u32) -> DeclorchResult<Vec<DailyBreakdown>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_first_event_date(&self) -> DeclorchResult<Option<String>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn query_today_cost(&self) -> DeclorchResult<f64> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    pub fn cleanup_old(&self, days: u32) -> DeclorchResult<usize> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
