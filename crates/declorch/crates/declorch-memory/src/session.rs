...
//! Session management — load/save conversation history.
//!
//! The `Session` data type is defined in `declorch-types` to avoid a dependency
//! on rusqlite for crates that only need the data structure.

pub use declorch_types::session::Session;

use chrono::Utc;
use declorch_types::agent::{AgentId, SessionId};
use declorch_types::error::{DeclorchError, DeclorchResult};
use declorch_types::message::{ContentBlock, Message, MessageContent, Role};
...
impl SessionStore {
        ...
        match result {
            Ok((agent_str, messages_blob, tokens, label)) => {
                ...
                let messages: Vec<Message> = rmp_serde::from_slice(&messages_blob)
                    .map_err(|e| DeclorchError::Serialization(e.to_string()))?;
                Ok(Some(Session {
                    id: session_id,
                    agent_id,
                    messages,
    ...
    pub fn list_sessions(&self) -> DeclorchResult<Vec<serde_json::Value>> {
        ...
        for row in rows {
            sessions.push(row.map_err(|e| DeclorchError::Memory(e.to_string()))?);
        }
        Ok(sessions)
    }
...
impl SessionStore {
    ...
    pub fn load_canonical(&self, agent_id: AgentId) -> DeclorchResult<CanonicalSession> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
    ...
    ) -> DeclorchResult<CanonicalSession> {
        let mut canonical = self.load_canonical(agent_id)?;
            ...
            if to_compact > canonical.compaction_cursor {
                ...
                for msg in compacting {
                    let role = match msg.role {
                        declorch_types::message::Role::User => "User",
                        declorch_types::message::Role::Assistant => "Assistant",
                        declorch_types::message::Role::System => "System",
                    };
                    let text = msg.content.text_content();
                    if !text.is_empty() {
                        // Truncate individual messages in summary to keep it compact (UTF-8 safe)
                        let truncated = if text.len() > 200 {
                            format!("{}...", declorch_types::truncate_str(&text, 200))
                        } else {
                            text
                        };
    ...
    ) -> DeclorchResult<(Option<String>, Vec<Message>)> {
    ...
    fn save_canonical(&self, canonical: &CanonicalSession) -> DeclorchResult<()> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| DeclorchError::Internal(e.to_string()))?;
                        ...
                        match block {
                            ...
                            ContentBlock::Thinking { thinking, .. } => {
                                text_parts.push(format!(
                                    "[thinking: {}]",
                                    declorch_types::truncate_str(thinking, 200)
                                ));
                            }
...
mod tests {
    ...
    fn test_jsonl_mirror_write() {
        ...
        session
            .messages
            .push(declorch_types::message::Message::user("Hello"));
        session
            .messages
            .push(declorch_types::message::Message::assistant("Hi there!"));
        store.save_session(&session).unwrap();
