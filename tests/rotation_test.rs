...
use mycute::utils::crypto::{encrypt, decrypt};
...
async fn test_key_rotation_full_cycle() -> anyhow::Result<()> {
    ...
    let config_manager = Arc::new(ConfigManager::new(None, Some(tmp_path.to_string_lossy().to_string())));
    ...
    db.execute(Statement::from_string(DatabaseBackend::Sqlite, create_table_sql)).await?;
    ...
    let delete_query = sea_orm::sea_query::Query::delete()
        ...
        .and_where(sea_orm::sea_query::Expr::col(sea_orm::sea_query::Alias::new("key")).eq(test_key_name))
        .to_owned();
    ...
    assert!(updated_settings.server.last_rotated_at.is_some(), "Timestamp should be updated");
    
    // Verify Config Re-encryption
    let rotated_my_sec = updated_settings.my_sec.as_ref().expect("my_sec should exist");
    let decrypted_my_sec = decrypt(rotated_my_sec, new_key)?;
    assert_eq!(decrypted_my_sec, secret_val, "Config data should be correctly re-encrypted");
    ...
    let row = db.query_one(builder.build(&select_query))
        .await?
    ...
    assert_eq!(decrypted_db_val, db_secret, "DB data should be correctly re-encrypted");
    
    // Verify OLD key fails
    assert!(decrypt(&rotated_db_val, initial_key).is_err(), "Old key should no longer work for decryption");
