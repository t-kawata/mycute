...
use std::sync::Arc;
use mycute::{
    constants::{HEADER_X_MYCUTE_TIMESTAMP, HEADER_X_MYCUTE_SIGNATURE, HEADER_X_MYCUTE_SENDER_PUBKEY, HEADER_X_MYCUTE_CA_BASE_URL},
    mode::rt::middleware::p2p_clock_sync_enforcement_middleware::p2p_clock_sync_enforcement_middleware,
    utils::{db::DbPools, time, crypto::{Ed448KeyValuePair}},
    stt_config::ConfigManager,
    mode::rt::client::secure_client::SecureClient,
    mode::rt::rtbl::blacklists_bl::{self, CrimeEvidence, CrimeDetail},
};
...
async fn test_p2p_bypass_no_header() {
    ...
    let app = Router::new()
        ...
        .layer(axum::middleware::from_fn(p2p_clock_sync_enforcement_middleware))
        .layer(Extension(config.clone()))
        .layer(Extension(db_arc.clone()))
        .layer(Extension(Arc::new(SecureClient::new(reqwest::Client::new(), db_arc, config))));
...
async fn test_p2p_self_enforcement() {
    ...
    let app = Router::new()
        ...
        .layer(Extension(config.clone()))
        .layer(Extension(db_arc.clone()))
        .layer(Extension(Arc::new(SecureClient::new(reqwest::Client::new(), db_arc.clone(), config.clone()))));
    ...
    blacklists_bl::add_to_blacklist(db_arc.as_ref(), evidence).await.unwrap();
    ...
    let req = Request::builder()
        ...
        .header(HEADER_X_MYCUTE_SENDER_PUBKEY, hex::encode(other_keypair.public))
        .header(HEADER_X_MYCUTE_CA_BASE_URL, "http://ca.example.com")
        .body(Body::empty())
        .unwrap();
...
async fn test_p2p_blacklisted_peer() {
    ...
    let app = Router::new()
        ...
        .layer(Extension(config.clone()))
        .layer(Extension(db_arc.clone()))
        .layer(Extension(Arc::new(SecureClient::new(reqwest::Client::new(), db_arc.clone(), config))));
    ...
    blacklists_bl::add_to_blacklist(db_arc.as_ref(), evidence_valid).await.unwrap();
