...
use mycute::utils::crypto::{Ed448KeyValuePair, Ed448Signature, verify_signature};

#[test]
fn test_ed448_linkage() {
    let identity = ExtendedPoint::identity();
    assert_eq!(identity.compress().0, ExtendedPoint::identity().compress().0);
}
...
fn test_keygen_sign_verify_flow() {
    ...
    let valid_bad_msg = verify_signature(&pair.public, bad_msg, &sig).expect("Verify function failed");
    assert!(!valid_bad_msg, "Signature should be invalid for different message");
    
    // 5. Verify (Bad Key)
    let pair2 = Ed448KeyValuePair::generate().expect("Keygen2 failed");
    let valid_bad_key = verify_signature(&pair2.public, message, &sig).expect("Verify function failed");
    assert!(!valid_bad_key, "Signature should be invalid for different key");
    
    // 6. Verify (Tampered Signature)
    let mut bad_sig_bytes = sig.signature;
    bad_sig_bytes[0] ^= 0xFF; // Flip first byte of R
    let bad_sig = Ed448Signature { signature: bad_sig_bytes };
    let valid_tamper = verify_signature(&pair.public, message, &bad_sig).expect("Verify function failed");
...
fn test_derive_public_key() {
    ...
    assert_eq!(pair.public, recovered.public, "Public key should be deterministically derived from secret");
    ...
    assert!(verify_signature(&pair.public, msg, &sig).expect("Verify failed"), "Signature from recovered key should be valid for original public key");
}
...
fn test_file_sign_verify() {
    ...
    let file_path = dir.join("test_sig_ed448.txt");
    {
        ...
        f.write_all(b"File content to sign").expect("Failed to write to temp file");
    }
...
fn test_key_save_load() {
    use mycute::utils::crypto::{save_keypair, load_keypair};
