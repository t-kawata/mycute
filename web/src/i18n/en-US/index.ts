// en-US
export default {
  app: {
    fab: {
      overlay: {
        on: "Close Overlay",
        off: "Overlay View",
        openHistory: "Open Voice Input History",
        sttHistoryEmpty: "No history",
        sttHistoryCopied: "Copied to clipboard",
      },
      pinDuringVoice: {
        on: "Pin During Voice Input",
        off: "No Pin During Voice Input",
      },
      logout: "Logout",
      restart: "Restart",
      shutdown: "Shutdown",
    },
    settings: {
      englishMode: "英語モード",
      englishModeDescription:
        "このスイッチをONにするとアプリ全体が英語になります。OFFにすると日本語になります。",
      sttEngine: "STT Engine",
      sttEngineDescription: "Select the speech recognition engine to use.",
      sttEngineOs: "OS Native",
      sttEngineOpenAI: "OpenAI",
      llmSettings: "LLM Settings",
      llmSettingsDescription:
        "Configure the Large Language Models (LLM) used for AI interaction. Multiple settings will be used in round robin.",
      llmName: "Name",
      llmBaseUrl: "Base URL",
      llmApiKey: "API Key",
      llmModel: "Model",
      llmAdd: "Add LLM Endpoint",
      danger: "Danger Zone",
      dangerDescription:
        "Operations here are irreversible, including application reset.",
      resetApplication: "Reset Application",
      resetConfirm:
        "Are you sure you want to reset the application? This action cannot be undone, and all data will be permanently lost.",
      resetFailed: "Failed to reset application.",
      resetAndExit: "Complete Reset and Exit",
      resetAndExitConfirm:
        "Complete Reset will permanently delete the following:\n\n- All application data (home directory)\n- Your public/private key pair (identity)\n- Your personal identification on the network\n\nThis operation CANNOT be undone. After re-setup, a new identity will be issued and your previous identity CANNOT be restored.\n\nAre you sure you want to proceed?",
      resetAndExitFailed: "Failed to complete reset and exit.",
      cleanupCmdForWindows: "Get Cleanup Command",
      cleanupCmdCopied: "Cleanup command copied",
      ownerActivation: "Owner Activation",
      ownerPassphrase: "Owner Passphrase",
      activate: "Activate",
      ownerModeActivated: "Owner Mode Activated.",
      ownerModeDeactivated: "Owner Mode Deactivated.",
      invalidPassphrase: "Invalid Passphrase.",
      ownerModeActive: "Owner Mode Active",
      rootAuthority: "You have Owner Authority.",
      caModeActive: "CA Active",
      caAuthorized: "You are authorized as a Central Authority (CA).",
      myPubKey: "Your Public Key",
      copyPubKey: "Public key copied to clipboard",
      genCaToken: "Generate CA Cert",
      genCaTokenDescription: "Generate a CA Cert from the target public key.",
      regCaToken: "Register CA Cert",
      regCaTokenDescription:
        "Register CA Cert and start as a Central Authority.",
      verifyCaToken: "Verify CA Cert",
      verifyCaTokenDescription: "Verify a CA Cert and check its validity.",
      myCaToken: "Your CA Appointment Cert",
      unregCaTokenConfirm:
        "Are you sure you want to delete the CA Appointment Cert and stop the functional CA?",
      unregCaTokenSuccess: "CA Appointment Cert deleted successfully.",
      genCaTokenDialogTitle: "Generate CA Cert",
      targetPubKey: "Target Public Key (Hex)",
      targetPubKeyHint: "Please enter the target node's public key (Hex).",
      expireHours: "Expire Hours",
      expireHoursHint: "Expiration time (hours, default 336h = 14 days).",
      issueAndCopy: "Issue and Copy",
      enterPubKey: "Please enter the public key.",
      enterValidHours: "Please enter valid hours.",
      genCaTokenSuccess: "CA Cert copied to clipboard.",
      genCaTokenFail: "Failed to generate CA Cert.",
      verifyCaTokenDialogTitle: "Verify CA Cert",
      enterCaToken: "Please enter the CA Cert.",
      verify: "Verify",
      caTokenInputLabel: "CA Cert String",
      verificationResult: "Verification Result",
      caPubKey: "CA Public Key",
      expireAt: "Expire At",
      tokenValid: "This CA Cert is VALID.",
      tokenInvalid: "This CA Cert is INVALID.",
      verifyCaTokenSuccess: "Verification successful.",
      verifyCaTokenFail: "Verification failed.",
      regCaTokenDialogTitle: "Register CA Cert",
      register: "Register",
      registerCaTokenSuccess: "CA Cert registered successfully.",
      registerCaTokenFail: "Failed to register CA Cert.",
      copyPubKeySuccess: "CA Public Key copied to clipboard.",
      unregCaToken: "Delete CA Appointment Cert",
      myLicenses: "Your Licenses",
      myLicensesUnit: "license(s)",
      noLicenses: "No licenses registered.",
      licenseValid: "Valid",
      licenseExpired: "Expired",
      copyLicense: "License copied",
      unregisterLicense: "Delete License",
      unregisterLicenseConfirm:
        "Are you sure you want to delete this license? This action cannot be undone.",
      licenseUnregistered: "License removed.",
      regLicense: "Register License",
      regLicenseDescription: "Register a license received from a CA.",
      verifyLicense: "Verify License",
      verifyLicenseDescription: "Verify a license string's validity.",
      licenseInputLabel: "License String",
      licenseRegistered: "License registered successfully.",
      licenseRegisterFailed: "Failed to register license.",
      genLicense: "Issue License",
      genLicenseDescription: "Issue a license as a CA.",
      genLicenseDialogTitle: "Issue License",
      issueLicense: "Issue and Copy",
      genLicenseSuccess: "License issued and copied to clipboard.",
      genLicenseFail: "Failed to issue license.",
      permissions: "Grant Permissions",
      grantedPermissions: "Granted Permissions",
      expireExceedsCaTitle: "Expiration Limit",
      expireExceedsCaMessage:
        "The requested duration exceeds the CA token's expiration ({caExpire}). Would you like to adjust it to the maximum possible {maxHours} hours?",
      verifyLicenseSuccess: "This license is VALID.",
      verifyLicenseFail: "This license is INVALID.",
    },
    llm: {
      title: "LLM Management (LMGW)",
      settingsTab: "Settings",
      stepProviderSelect: "Select Provider",
      stepApiKey: "API Key Management",
      stepSync: "Save / Apply",
      syncSuccess: "Configuration saved and applied successfully.",
      syncDescription:
        'Click "Save & Apply" to encrypt and store new API keys in the database and apply the configuration to the system.',
      saveFailed: "Failed to save.",
      fetchFailed: "Failed to fetch settings.",
      newKey: "New (Unsaved)",
      storedKey: "Stored (Encrypted)",
      labelProviderLabel: "Display Name",
      labelApiKey: "API Key",
      labelApiKeyPlain: "API Key (Plain)",
      labelApiKeyEncrypted: "API Key (Encrypted)",
      labelWeight: "Weight",
      btnAddKey: "Add Key",
      deleteKeyTooltip: "Delete this key",
      apiKeyMask: "•••••••• (Stored)",
      btnSync: "Save & Apply",
      stepTest: "Communication Test",
      testDescription:
        "If at least one API key is registered, you can select a message below to perform a communication test.",
      testFail: "Communication test failed.",
      testResult: "Response from provider:",
      btnTest: "Send Test",
      testCommFail: "Communication failed.",
      testInvalidFormat: "Invalid response format.",
      testParseFail: "Failed to parse JSON.",
      testPresets: [
        "Hello! How are you feeling today?",
        "Please tell me the answer to 1+1.",
        "What is your favorite color?",
        "Please give me a brief self-introduction.",
        "What is your favorite movie genre?",
        "How is the weather in Tokyo today?",
        "What is your favorite food?",
        "Tell me a funny joke.",
        "Tell me a tongue twister!",
        "Which do you prefer, dogs or cats?",
        "Is there any interesting news lately?",
        "What did you have for breakfast?",
        "What are your hobbies?",
        "What do you think about the future of AI?",
        "Please give me a simple math problem.",
        "What are some tips for programming?",
        "Where is your recommended travel destination?",
        "When is your favorite season?",
        "Tell me a famous quote.",
        "Please recommend a book.",
        "What should I do when I can't sleep?",
        "Please predict tomorrow's lucky color.",
        "Where would you go if you could time travel?",
        "Which do you prefer, coffee or tea?",
        "What do you think about the universe?",
      ],
    },
    common: {
      cancel: "Cancel",
      reset: "Reset",
      delete: "Delete",
      close: "Close",
      ok: "OK",
      next: "Next",
      back: "Back",
      error: "Error",
    },
  },
  page: {
    login: {
      signin: "Sign In",
      signup: "Sign Up",
      reset: "Reset Up",
      createAccount: "Create Account",
      login: "Login",
      registerVdrKey: "Register Vendor Key",
      error: {
        failedToSignIn: "Invalid authentication information",
        failedToSignUp: "Invalid input content",
        failedToValidateVdrKey: "Invalid VDR-KEY",
        requiredVdrKey: "Please enter VDR-KEY",
        autoSetupFailed: "Auto Setup Failed",
        comingSoon: "Under preparation",
      },
    },
    index: {
      calendar: {
        ok: "OK",
        ng: "NG",
      },
      search: {
        ok: "OK",
        ng: "NG",
        salary: "Job Salary",
      },
      friends: {
        give: "GIVE",
        none: "NONE",
        point: "Gift Point",
      },
    },
  },
};
