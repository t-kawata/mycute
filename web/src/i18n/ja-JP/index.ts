...
export default {
  app: {
    ...
    settings: {
      ...
      unregisterLicenseConfirm: 'このライセンスを削除しますか？この操作は取り消せません。',
      ...
      expireExceedsCaMessage: '指定された期限はCA任命証の期限（{caExpire}）を超えています。最大で発行可能な {maxHours} 時間に修正しますか？',
    ...
    llm: {
      ...
      syncDescription: '「保存して反映」を押すと、新規APIキーがバックエンドで暗号化されてDBに保存され、最新の設定がシステムに反映されます。',
      ...
      btnSync: '保存して反映',
      stepTest: '通信テスト',
      testDescription: '使用可能なAPIキーが登録されている場合、以下のメッセージから選んで送信テストを行うことができます。',
