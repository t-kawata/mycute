<template>
  <div class="__mycute-glass-app-container q-pa-sm">
    <div class="__mycute-glass-panel-inner">
      <WaterRipple />
      <div
        class="__mycute-tabpanel-container __mycute-tabpanel-container-settings __mycute-tabpanel-container-settings-app"
        style="
          flex: 1;
          padding: 0;
      ...
      >
        <q-list>
          ...
          >
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.ownerModeActive")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.rootAuthority")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- オーナー表示 end ---------------->
          ...
          <q-item
            ...
            clickable
            @click="mainStore.setIsGenCaTokenDialogOpen(true)"
          ...
          >
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.genCaToken")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.genCaTokenDescription")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- 一行 end ---------------->
          ...
          >
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.caModeActive")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.caAuthorized")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- CA表示 end ---------------->
          ...
          >
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.myCaToken")
              }}</q-item-label>
              <q-item-label caption class="ellipsis text-dark">{{
                mainStore.caToken
              }}</q-item-label>
            </q-item-section>
            <q-item-section v-if="mainStore.isOwnerActive" side>
              <q-btn
                ...
                color="negative"
                icon="delete_forever"
                @click.stop="onUnregCaTokenClicked"
              />
            </q-item-section>
          </q-item>
          <!-------------- 一行 end ---------------->
          ...
          <q-item
            ...
            class="q-px-none q-mt-sm"
            clickable
            @click="mainStore.setIsGenLicenseDialogOpen(true)"
            style="user-select: none"
          >
            <q-item-section avatar>
              <q-avatar color="dark" text-color="white">
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.genLicense")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.genLicenseDescription")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- ライセンスの発行 end ---------------->
          ...
          <q-item
            ...
            class="q-px-none q-mt-sm"
            clickable
            @click="mainStore.setIsVerifyLicenseDialogOpen(true)"
            style="user-select: none"
          >
            <q-item-section avatar>
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.verifyLicense")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.verifyLicenseDescription")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- ライセンス検証ボタン end ---------------->
          ...
          <q-item
            ...
            class="q-px-none q-mt-sm"
            clickable
            @click="mainStore.setIsRegisterLicenseDialogOpen(true)"
            style="user-select: none"
          >
            <q-item-section avatar>
              <q-avatar color="app" text-color="white">
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.regLicense")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.regLicenseDescription")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- ライセンス登録ボタン end ---------------->
          ...
          <q-expansion-item
            key="licenses-expansion"
            v-model="isLicensesExpanded"
            class="q-px-none"
            header-class="q-px-none"
            @show="onLicenseExpand"
          >
            <template v-slot:header>
              ...
              <q-item-section>
                <q-item-label class="text-dark text-weight-bold">{{
                  t("app.settings.myLicenses")
                }}</q-item-label>
                <q-item-label caption class="text-dark"
                  >{{ mainStore.licenses.length }}
                  {{ t("app.settings.myLicensesUnit") }}</q-item-label
                >
              </q-item-section>
            </template>
            <!-- ライセンス一覧コンテンツ -->
            <div class="q-mt-sm q-mb-md">
              <div
                v-if="mainStore.licenses.length === 0"
                class="text-center text-caption q-pa-md text-grey-8"
              >
                {{ t("app.settings.noLicenses") }}
              </div>
              <div
                v-for="lic in mainStore.licenses"
                :key="lic.id"
                class="q-mx-sm q-mb-md q-pa-md bg-app text-dark relative-position shadow-6 __mycute-settings-license-card"
                style="
                  border-radius: 16px;
              ...
              >
                <!-- ツールボタン (Floating) -->
                <div
                  class="absolute-top-right q-pa-sm flex q-gutter-xs"
                  style="z-index: 2"
                >
                  <q-btn
                    flat
                  ...
                  <q-btn
                    ...
                    color="dark"
                    icon="delete_forever"
                    @click="onUnregisterLicense(lic.id)"
                  />
                </div>
                ...
                <q-list class="relative-position" style="z-index: 1">
                  ...
                  <q-item class="q-px-none">
                    <q-item-section>
                      <q-item-label
                        ...
                        >{{ t("app.settings.expireAt") }}</q-item-label
                      >
                  ...
                  <q-item class="q-px-none">
                    <q-item-section>
                      <q-item-label
                        caption
                        class="text-dark text-weight-bold"
                        >{{ t("app.settings.caPubKey") }}</q-item-label
                      >
                  ...
                  <div v-if="lic.permissions" class="q-mt-sm">
                    <div class="text-caption text-dark text-weight-bold">
                      {{ t("app.settings.grantedPermissions") }}
                    </div>
                    <pre
                      class="bg-dark q-pa-sm q-ma-none q-mt-xs text-caption text-white"
                      style="
                        border-radius: 6px;
                        overflow: auto;
                        max-height: 120px;
                      ...
                      "
                      >{{ JSON.stringify(lic.permissions, null, 4) }}</pre
                    >
                  </div>
                </q-list>
              </div>
          ...
          <q-item
            ...
            class="q-px-none q-mt-sm"
            clickable
            @click="mainStore.setIsVerifyCaTokenDialogOpen(true)"
            style="user-select: none"
          >
            <q-item-section avatar>
              <q-avatar color="app" text-color="white">
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.verifyCaToken")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.verifyCaTokenDescription")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- CA任命証の検証 end ---------------->
          ...
          <q-item
            ...
            class="q-px-none q-mt-sm"
            clickable
            @click="mainStore.setIsRegisterCaTokenDialogOpen(true)"
            style="user-select: none"
          >
            <q-item-section avatar>
              <q-avatar color="app" text-color="white">
            ...
            <q-item-section>
              <q-item-label class="text-dark text-weight-bold">{{
                t("app.settings.regCaToken")
              }}</q-item-label>
              <q-item-label caption class="text-dark">{{
                t("app.settings.regCaTokenDescription")
              }}</q-item-label>
            </q-item-section>
          </q-item>
          <!-------------- CA任命証の登録 end ---------------->
          ...
          >
            <template v-slot:header>
              ...
              <q-item-section>
                <q-item-label color="negative">{{
                  t("app.settings.danger")
                }}</q-item-label>
                <q-item-label caption>{{
                  t("app.settings.dangerDescription")
                }}</q-item-label>
              </q-item-section>
            </template>
            <!-- Danger コンテンツ -->
            <div class="q-mt-sm q-mb-md">
              <q-btn
                ...
                color="negative"
                icon="restore"
                :label="t('app.settings.resetApplication')"
              ...
              <q-btn
                ...
                color="dark"
                icon="delete_forever"
                :label="t('app.settings.resetAndExit')"
                @click="mainStore.setIsResetAndExitConfirmOpen(true)"
              />
            </div>
          </q-expansion-item>
          <!-------------- 一行 end ---------------->
