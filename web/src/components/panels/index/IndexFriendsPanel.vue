<template>
  <q-toolbar :class="['text-primary', 'bg-white', { 'fixed-top': !IS_TAURI_DESKTOP }]" style="z-index: 2;">
    <HeartIcon class="__harunohi-btn-icon __harunohi-header-btn-icon __harunohi-header-btn-icon-heart" style="margin-right: 3px;" />
    <GaveBadgeCounter :total="mainStore.subGaveBadgeTotal" :lines="lines" :item-width="segWidth" :width="counterBarWidth - 3" :height="23" :active="isGaveBadgeCounterActive" />
    <q-space />
  ...
  <div class="__harunohi-tabpanel-container" :style="`margin-top: ${IS_TAURI_DESKTOP ? 0 : 34}px; height: calc(100dvh - ${IS_TAURI_DESKTOP ? 136 : 70}px)`">
    <SwipeActions2
      :candidates="mainStore.candidates"
      @swipe-out="handleSwipeOut"
      @long-press="handleLongPress"
    >
      <!-- 左スロット -->
      <template #left="{ candidate, swipeDistance }">
        <div style="width: calc(100% - 15px); text-align: right; color: #ffffff; font-size: 35px; font-weight: bold; padding-right: 15px;">
          {{ t('page.index.friends.none') }}
        </div>
      </template>

      <!-- 右スロット -->
      <template #right="{ candidate, swipeDistance }">
        <div style="width: calc(100% - 15px); text-align: left; color: #ffffff; font-size: 35px; font-weight: bold; padding-left: 15px;">
          {{ t('page.index.friends.give') }}
        </div>
      </template>
    </SwipeActions2>
    <div class="absolute-bottom-right relative-position __harunohi-gamaguchi2-wrapper">
      <GaveBadgeGamaguchiIcon :label="t('page.index.friends.point')" :total="mainStore.subPointTotal" :class="[
        'relative-position',
        '__harunohi-gamaguchi2-icon',
        '__harunohi-icon-secondary-light',
...
import SwipeActions2 from 'src/components/tools/SwipeActions2.vue'
import { BadgeCandidate, type SwipeDirection } from 'src/models/main'
import SearchIcon from 'src/components/icons/SearchIcon.vue'
import HeartIcon from 'src/components/icons/HeartIcon.vue'
import GaveBadgeGamaguchiIcon from 'src/components/icons/GaveBadgeGamaguchiIcon.vue'
import MovingStar from 'src/components/decorations/MovingStar.vue'
import GaveBadgeCounter from 'src/components/decorations/GaveBadgeCounter.vue'
...
const lines = computed(() => Math.ceil(mainStore.subGaveBadgeTotal / maxSegmentsPerLine.value))
const isGaveBadgeCounterActive = ref(false)
const isGamaguchiPurun = ref(false)

const handleSwipeOut = async ({ candidate, direction }: { candidate: BadgeCandidate, direction: SwipeDirection }) => {
  console.log(`${candidate.title} を ${direction} にスワイプしました`);
  if (direction === 'like' as SwipeDirection) {
    isStarMoveStart.value = true;
    (async () => { // バッジカウンターのウェーブを制御
      await sleep(650)
      isGaveBadgeCounterActive.value = true
      await sleep(2000)
      isGaveBadgeCounterActive.value = false
    })();
    (async () => { // がま口財布アイコンのぷるんを制御
      await sleep(650)
      isGamaguchiPurun.value = true
      // ここでイベントカレンダーデータに「仮」で追加
      mainStore.pushGaveBadgeByCandidate(candidate)
      await sleep(2000)
...
const handleLongPress = ({ candidate }: { candidate: BadgeCandidate }) => {
...
.__harunohi-balloon-element {
  ...
  &-purun {
    animation: purun2 0.8s linear 0s 2;
  }
}
