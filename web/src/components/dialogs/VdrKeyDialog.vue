      ...
      <div class="__mycute-login-page-form-area relative-position">
        ...
        <div class="absolute-center full-width">
          ...
          <q-btn
            ...
            unelevated
            color="purple"
            :label="t('page.login.registerVdrKey')"
            class="q-mx-xl"
...
const onFocusEmail = () => {
  vdrKeyOutlined.value = true;
};
const onBlurEmail = () => {
  vdrKeyOutlined.value = false;
};
...
const runAutoSetup = async () => {
  ...
  try {
    ...
    let apxTokenObj = {};
    try {
      apxTokenObj = JSON.parse(apxAuthRes.body);
    } catch (e) {
      throw new Error("Failed to parse APX token");
    }
    ...
    const vdr100YearToken = await createVdr100YearToken(
      apxToken,
      newVdrKey,
      apxId,
      vdrId,
    ...
    if (!vdr100YearToken)
      throw new Error("Failed to create VDR 100 Year Token");
