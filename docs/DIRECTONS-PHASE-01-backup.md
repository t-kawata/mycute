...
1.  **パス解決 (`resolve_data_directories` 再現)**
    ...
    -   *実装例*:
        ```go
        client, _ := s3client.NewS3Client(..., true) // useLocal=true
        // Walkメソッド等を使ってファイルをリストアップ
        ```
    ...
    **方法1: OpenAI Embeddings API経由（推奨）**
    ...
    func NewEmbedder(ctx context.Context) (embeddings.Embedder, error) {
        return openai.NewOpenAI(
            ...
            openai.WithToken(os.Getenv("OPENAI_API_KEY")),
            openai.WithModel("text-embedding-3-small"), // Bifrostが対応しているモデル
        )
    }
...
2.  **コンテキスト検索 (`get_context` 再現)**
    ...
    -   **グラフ探索**: 取得した `Chunk` に関連する `Node` や `Edge` を `GraphStorage.GetContext` で取得。
    -   *根拠*: Python版 `search.py` -> `get_context` がベクトル検索とグラフ探索を組み合わせているため。
