...
impl SpeechRecognizer {
    ...
    pub fn new(
        ...
        let mac_backend = if engine == SttEngine::Os {
            ...
            match MacSpeechBackend::new(tx.clone(), engine.clone(), locale, pc_backend, pc_config, flat_replaces.clone()) {
        ...
        let win_backend = if engine == SttEngine::Os {
            ...
            match WinSpeechBackend::new(tx.clone(), locale, pc_backend, pc_config, flat_replaces.clone()) {
                Ok(backend) => Some(backend),
                Err(e) => {
