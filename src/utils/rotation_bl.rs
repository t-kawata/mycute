use std::sync::Arc;
use sea_orm::{DatabaseConnection, ConnectionTrait, TransactionTrait, sea_query::{Alias, Expr, Query}};
...
pub async fn check_and_rotate_keys(
    ...
    let should_rotate = if let Some(s) = last_rotated_at.as_deref() {
        ...
        if let Ok(last) = DateTime::parse_from_rfc3339(s) {
             ...
             if diff.num_days() < rotation_days as i64 {
                 log::info!("Key rotation not needed. (Last: {}, Limit: {} days)", last, rotation_days);
                 false
             } else {
                 true
             }
             ...
             if diff.num_days() < rotation_days as i64 {
                 log::info!("Key rotation not needed. (Last: {}, Limit: {} days)", last_naive, rotation_days);
                 false
             } else {
                 true
             }
    ...
    for target in targets {
        match target {
            CryptoTarget::Db { table_name, col_name, pk_col } => {
    ...
    // 7. 設定ファイルの保存
    config_manager.save().map_err(|e| anyhow::anyhow!("Failed to save config: {}", e))?;
