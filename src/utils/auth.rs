...
impl Drop for BackendProcessGuard {
    fn drop(&mut self) {
        ...
        {
            use std::process::Command;
            let _ = Command::new("kill").arg("-9").arg(self.pid.to_string()).spawn();
        }
...
fn spawn_elevated_server_macos(args: &[&str]) -> anyhow::Result<u32> {
    ...
    let cmd_str = format!("'{}' {} > {} 2>&1 & echo $!", exe_str.replace("'", "'\\''"), args_str, log_file);
    let script = format!("do shell script \"{}\" with administrator privileges", cmd_str.replace("\"", "\\\""));
    
    let output = Command::new("osascript").arg("-e").arg(&script).output()?;
    if !output.status.success() {
            // キャンセルされた場合などもここに来る
            anyhow::bail!("Elevation failed (User cancelled or error): {}", String::from_utf8_lossy(&output.stderr));
    }
    let pid_str = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let pid: u32 = pid_str.parse().map_err(|_| anyhow::anyhow!("Invalid PID returned: {}", pid_str))?;
...
fn spawn_elevated_server_windows(args: &[&str]) -> anyhow::Result<u32> {
    ...
    if !output.status.success() {
         anyhow::bail!("Elevation failed: {}", String::from_utf8_lossy(&output.stderr));
    }
    
    let pid_str = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let pid: u32 = pid_str.parse().map_err(|_| anyhow::anyhow!("Invalid PID: {}", pid_str))?;
    Ok(pid)
}
