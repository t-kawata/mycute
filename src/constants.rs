...
/// MYCUTE OS のバージョン番号。
pub const MYCUTE_VERSION: &str = concat!("v", env!("CARGO_PKG_VERSION"));
...
/// 削除キー送信後のクールダウン時間（ミリ秒）。
/// IMEの自動確定を防ぐための最小待機時間です。
/// 以前は250msでしたが、プロトコル改善により30msまで短縮されました。
pub const DELETION_COOLDOWN_MS: u64 = 30;
