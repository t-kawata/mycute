...
impl PunctuationMachine {
    ...
    pub fn insert_with_context(&self, text: &str, context: &str, locale: &LocaleCode, allow_terminal_punctuation: bool) -> Result<String> {
        if text.is_empty() {
    ...
    fn is_sentence_starter(&self, token: &TokenInfo) -> bool {
        let starters = [
            "はい", "ええ", "うん", "いや", "まあ", "さて", "そう", "でも", "しかし", "ただ", "じゃあ", "では",
            "じゃ", "それ", "あと", "もう", "また", "そして", "だから",
        ];
    ...
    fn should_insert_period_ja(&self, index: usize, tokens: &[TokenInfo], allow_terminal_punctuation: bool) -> bool {
        ...
        if current.pos == "助詞" && ["接続助詞", "格助詞"].contains(&current.pos_detail1.as_str()) {
             if ["が", "けど", "けれど", "けれども", "し", "から", "ので", "のに", "て", "で"].contains(&current.surface.as_str()) {
                 return false;
             }
        }
        ...
        if (current.pos == "動詞" || current.pos == "形容詞" || current.pos == "助動詞") && 
           (current.conjugation_form.contains("基本形") || current.conjugation_form.contains("タ形")) {
    ...
    fn should_insert_question_ja(&self, index: usize, tokens: &[TokenInfo], allow_terminal_punctuation: bool) -> bool {
        // 【最重要】ライブエッジ（最後尾）には絶対に打たない
        // ただし、allow_terminal_punctuation が true の場合（タイムアウト時）は許可する
