...
pub fn main_of_am(flgs: AMFlgs) -> anyhow::Result<()> {
    ...
    rt.block_on(async {
        ...
        // DB接続
        // ==============================
        let db = get_db(&env, &flgs.common.log_level).await
        ...
        // AutoMigration の実行
        // ==============================
        let rw_conn = db.get_rw().map_err(|e| anyhow::anyhow!("Failed to get RW connection for migration: {}", e))?;
        if flgs.fresh {
            log::info!("Running AutoMigration (Fresh: NUCLEAR)...");
            Migrator::fresh(rw_conn).await.map_err(|e| anyhow::anyhow!("Failed to fresh migrations: {}", e))?;
            ...
            Migrator::refresh(rw_conn).await.map_err(|e| anyhow::anyhow!("Failed to refresh migrations: {}", e))?;
            ...
            Migrator::up(rw_conn, None).await.map_err(|e| anyhow::anyhow!("Failed to run migrations: {}", e))?;
