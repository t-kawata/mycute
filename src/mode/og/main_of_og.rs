...
pub struct OGFlgs {
    ...
    #[arg(short = 'f', long = "file", required = true, help = "Path to the passphrases file (must contain exactly 15 lines)")]
    pub file: String,
}
...
pub fn main_of_og(flgs: OGFlgs) -> Result<()> {
    ...
    let passphrases: Vec<String> = reader.lines()
    ...
    if passphrases.len() != 15 {
        return Err(anyhow!("Invalid number of passphrases. Expected 15, found {}", passphrases.len()));
    }
    ...
    for (i, pass) in passphrases.iter().enumerate() {
        ...
        let key_array: [u8; 32] = key_bytes.as_bytes().try_into()
        ...
        blobs.push(blob);
        log::debug!("Passphrase #{}: Blob size {} bytes", i+1, blobs.last().unwrap().len());
    }
    ...
    for (i, pass) in passphrases.iter().enumerate() {
        ...
        if blob.len() < 1 + salt_len + 12 {
             return Err(anyhow!("Verification failed: Blob too short for index {}", i));
        }
        ...
        let salt = SaltString::from_b64(salt_str).map_err(|e| anyhow!("Salt parse error: {}", e))?;
        ...
        if plaintext != secret_bytes {
            return Err(anyhow!("Verification failed for passphrase #{}: Content mismatch", i+1));
        }
    }
    ...
    code.push_str(&format!("pub const OWNER_PUB_KEY_HEX: &str = \"{}\";\n\n", public_hex));
