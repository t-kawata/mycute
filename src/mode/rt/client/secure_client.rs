use crate::constants::{
    ERR_HTTP_CLIENT, HEADER_X_MYCUTE_CA_BASE_URL, HEADER_X_MYCUTE_SENDER_PUBKEY,
    HEADER_X_MYCUTE_SIGNATURE, HEADER_X_MYCUTE_TIMESTAMP, P2P_BLACKLIST_SYNC_TARGET_MAX,
...
impl SecureClient {
    ...
    pub async fn get_without_verification<U: reqwest::IntoUrl>(
        &self,
        url: U,
    ) -> Result<Response, ApiError> {
        let builder = self.inner.get(url);
    ...
    fn prepare_request(&self, mut builder: RequestBuilder) -> RequestBuilder {
        ...
        if let Ok(keypair) = self.config.get_node_keypair() {
            ...
            if let Ok(sig_raw) = keypair.sign(&msg) {
                ...
                builder =
                    builder.header(HEADER_X_MYCUTE_SENDER_PUBKEY, hex::encode(keypair.public));
            }
        }
    ...
    async fn verify_response(&self, res: &Response, my_ts: i64) -> Result<(), ApiError> {
        ...
        // 1. 基本的な時刻と署名の抽出
        let peer_ts = match headers
            .get(HEADER_X_MYCUTE_TIMESTAMP)
        ...
        let peer_sig_hex = match headers
            .get(HEADER_X_MYCUTE_SIGNATURE)
            .and_then(|v| v.to_str().ok())
        {
            Some(sig) => sig,
            None => return Ok(()),
        ...
        let peer_pub_hex = match headers
            .get(HEADER_X_MYCUTE_SENDER_PUBKEY)
            .and_then(|v| v.to_str().ok())
        {
            Some(pubkey) => pubkey,
            None => return Ok(()),
        ...
        if diff > TIMESTAMP_TOLERANCE_MS {
            log::warn!(
                "SecureClient: Time skew detected. Peer: {}, My: {}, Diff: {}",
                peer_ts,
                my_ts,
                diff
            );
            ...
            if let Ok(_) = blacklists_bl::validate_evidence(&evidence) {
                ...
                let sender_ca_urls: Option<Vec<String>> = headers
                    .get(HEADER_X_MYCUTE_CA_BASE_URL)
                    .and_then(|v| v.to_str().ok())
                    .map(|s| {
                        s.split(',')
        ...
        if let Some(ca_base_urls_str) = headers
            .get(HEADER_X_MYCUTE_CA_BASE_URL)
            .and_then(|v| v.to_str().ok())
        {
            let ca_urls: Vec<String> = ca_base_urls_str
                .split(',')
            ...
            if !ca_urls.is_empty() {
                ...
                tokio::spawn(async move {
                    for url in selected_urls {
                        ...
                        {
                            log::warn!(
                                "SecureClient: Background sync with CA {} failed: {}",
                                url,
                                e.to_string()
                            );
                        }
