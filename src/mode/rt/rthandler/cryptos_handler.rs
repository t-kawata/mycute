...
| KEY | TYPE | VALIDATION | DESCRIPTION |
| --- | --- | --- | --- |
| `key` | string | required, length>=5, length<=50, regex=^[a-zA-Z0-9-_]{5,50}$ | 検索用のユニークキー |
...
### ⚫︎ 注意点
- `key` は半角英数字、ハイフン、アンダーバーのみの5文字以上、50文字以下である必要がある。
"#;
#[utoipa::path(
    tag = TAG,
    put,
    ...
    params(
        ("key" = String, Path, description = "半角英数字とハイフンとアンダーバーのみの5文字以上、50文字以下"),
        ("apx_id" = u32, Path),
        ("vdr_id" = u32, Path),
    ),
    responses(
