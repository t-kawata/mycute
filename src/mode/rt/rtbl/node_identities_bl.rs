use crate::constants::{ST_BAD_GATEWAY, ST_INTERNAL_SERVER_ERROR};
use crate::{
    constants::{
        DATE_FORMAT_STANDARD, ED448_KEY_BYTES_LEN, ED448_SIGNATURE_BYTES_LEN, ERR_ANCHOR_KEY,
        ERR_CA_CONNECT, ERR_CA_PARSE, ERR_CA_RESPONSE, ERR_CA_TRUST_FAIL, ERR_CA_UNREACHABLE,
        ERR_IDENTITY, ERR_INVALID_CA_KEY, ERR_INVALID_CA_TOKEN, ERR_INVALID_RESP, ERR_INVALID_SIG,
        ERR_MY_PUBKEY, ERR_SAVE, ERR_SIGN, ERR_SIG_FAIL, ERR_TICKET_PARSE,
        ERR_VERIFICATION_PENDING, KEY_TICKET_CA_PUBKEY, KEY_TICKET_FORUM_DESC, KEY_TICKET_FORUM_ID,
        KEY_TICKET_FORUM_NAME, KEY_TICKET_INITIAL_BALANCE, PATH_CA_IDENTITIES_APPLY,
        PATH_CA_IDENTITIES_ENTRY, PATH_IDENTITIES_SYNC,
    },
    ...
    mode::rt::{
        ...
        rterr::rterr,
        rtreq::ca_identities_req::{ApplyIdentityCaReq, EntryIdentityCaReq, ExistingForumReq},
        rtreq::node_identities_req::{
            ApplyIdentityNodeReq, EntryIdentityNodeReq, SyncIdentityNodeReq,
        },
        rtres::{
            ca_identities_res::{ApplyIdentityCaRes, EntryIdentityCaRes, SyncIdentityCaRes},
            errs_res::ApiError,
            node_identities_res::{
                ApplyIdentityNodeRes, EntryIdentityNodeRes, GetIdentityNodeRes, SyncIdentityNodeRes,
            },
        },
    },
...
pub async fn entry_identity_node(
    ...
    let url = format!(
        ...
        PATH_CA_IDENTITIES_ENTRY
    );

    let res = client.post(&url, &ca_req).await.map_err(|e| {
        ApiError::new_system(
            ST_BAD_GATEWAY,
            ERR_CA_CONNECT,
            format!("Failed to connect to CA: {}", e),
        )
    })?;

    if !res.status().is_success() {
        let status =
            StatusCode::from_u16(res.status().as_u16()).unwrap_or(ST_INTERNAL_SERVER_ERROR);
        let text = res.text().await.unwrap_or_default();
        return Err(ApiError::new_system(
            status,
            ERR_CA_RESPONSE,
            format!("CA returned error: {}", text),
        ));
    }

    let ca_res: EntryIdentityCaRes = res
        .json()
        .await
    ...
    for ticket_str in &ca_res.tickets {
        let ticket_json: Value = serde_json::from_str(ticket_str).map_err(|e| {
            ApiError::new_system(
                ST_BAD_GATEWAY,
                ERR_TICKET_PARSE,
        ...
        let _ = ticket_json
            .get(KEY_TICKET_CA_PUBKEY)
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                ApiError::new_system(
                    ST_BAD_GATEWAY,
                    ERR_TICKET_PARSE,
                    "Ticket missing ca_pubkey.",
                )
            })?;
        let forum_id = ticket_json
            .get(KEY_TICKET_FORUM_ID)
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                ApiError::new_system(ST_BAD_GATEWAY, ERR_TICKET_PARSE, "Ticket missing forum_id.")
            })?;
        let forum_name = ticket_json
            .get(KEY_TICKET_FORUM_NAME)
            .and_then(|v| v.as_str())
            .unwrap_or("Unknown")
            .to_string();
        let forum_desc = ticket_json
            .get(KEY_TICKET_FORUM_DESC)
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        let initial_balance = ticket_json
            .get(KEY_TICKET_INITIAL_BALANCE)
            .and_then(|v| v.as_i64())
            .unwrap_or(0) as i32;

        let forum_id_str = forum_id.to_string();
        let forum_uuid_bytes = Uuid::parse_str(forum_id)
            .unwrap_or_default()
            .as_bytes()
            .to_vec();
        ...
        Box::pin(async move {
            ...
            for pt in &parsed_tickets {
                let existing_ticket = tickets::Entity::find()
                    ...
                    .filter(
                        tickets::Column::ForumId
                            .eq(Uuid::from_slice(&pt.forum_uuid_bytes).unwrap_or_default()),
                    )
                    .one(txn)
                    .await
                    .map_err(|e| {
                ...
                if let Some(record) = existing_ticket {
                    ...
                    ticket_model.insert(txn).await.map_err(|e| {
                        ApiError::new_system(
                            ST_INTERNAL_SERVER_ERROR,
                            rterr::ERR_DATABASE,
                            e.to_string(),
            ...
            // RwLockWriteGuard (settings) が .await の前に確実に解放されるように明示的なブロックを使用
            {
                ...
                let entry = payload
                    ...
                    .or_insert_with(CaEntry::default);
                ...
                for pt in &parsed_tickets {
                    entry
                        .forum_states
                        .entry(pt.forum_id_str.clone())
                        .or_insert(ForumState {
    ...
    log::info!(
        "<NodeIdentities> Processed tickets and updated my_rem for CA '{}' (Atomic).",
        ca_res.ca_base_url
    );
...
pub async fn apply_identity_node(
    ...
    let url = format!(
        "{}{}",
        req.ca_base_url.trim_end_matches('/'),
        PATH_CA_IDENTITIES_APPLY
    );

    log::debug!(
        "<NodeIdentities> apply_identity_node: Requesting CA at {}",
        url
    );
    let res = client.post(&url, &ca_req).await.map_err(|e| {
        ApiError::new_system(
    ...
    let ca_res: ApplyIdentityCaRes = res.json().await.map_err(|e| {
        ApiError::new_system(
            ST_BAD_GATEWAY,
            ERR_CA_PARSE,
...
pub async fn sync_identity_node(
    ...
    let res = client.get(&url).await.map_err(|e| {
        ApiError::new_system(
            ST_BAD_GATEWAY,
            ERR_CA_UNREACHABLE,
            format!("Failed to contact CA: {}", e),
        )
    })?;

    if res.status().as_u16() == StatusCode::ACCEPTED {
        log::info!(
            "<NodeIdentities> Sync: Verification is still pending for {}.",
            my_pubkey_hex
        );
        return Err(ApiError::new_system(
            StatusCode::ACCEPTED,
            ERR_VERIFICATION_PENDING,
            "CA verification is still pending. Please wait for approval.",
        ));
    }

    if !res.status().is_success() {
        return Err(ApiError::new_system(
            StatusCode::from_u16(res.status().as_u16()).unwrap_or(ST_BAD_GATEWAY),
            ERR_CA_RESPONSE,
            format!("CA returned error: {}", res.status()),
        ));
    }

    let ca_res: SyncIdentityCaRes = res.json().await.map_err(|e| {
        ApiError::new_system(
            ST_INTERNAL_SERVER_ERROR,
            ERR_CA_PARSE,
            format!("Failed to parse CA response: {}", e),
    ...
    if let (Some(sig_hex), Some(tok_hex)) = (&ca_res.signature, &ca_res.ca_token) {
        ...
        let owner_pub_bytes = hex::decode(OWNER_PUB_KEY_HEX).map_err(|_| {
            ApiError::new_system(
                ST_INTERNAL_SERVER_ERROR,
                ERR_ANCHOR_KEY,
                "Invalid hardcoded owner key.",
            )
        })?;
        ...
        let ca_pub_bytes = hex::decode(&ca_res.ca_pubkey).map_err(|_| {
            ApiError::new_system(
                ST_BAD_GATEWAY,
                ERR_INVALID_CA_KEY,
                "Invalid ca_public_key hex.",
            )
        })?;
        ...
        let ca_sig_bytes = hex::decode(ca_token_sig_hex).map_err(|_| {
            ApiError::new_system(
                ST_BAD_GATEWAY,
                ERR_INVALID_CA_TOKEN,
                "Invalid ca_token signature hex.",
            )
        })?;
        ...
        if !verify_signature(&owner_pub_arr, &ca_msg, &ca_sig_struct).unwrap_or(false) {
            return Err(ApiError::new_system(
                ST_BAD_GATEWAY,
                ERR_CA_TRUST_FAIL,
                "CA Token verification failed (L1).",
            ));
        }

        let mut my_pub_arr = [0u8; ED448_KEY_BYTES_LEN];
        let my_pub_vec = hex::decode(&my_pubkey_hex).map_err(|_| {
            ApiError::new_system(
                ST_INTERNAL_SERVER_ERROR,
                ERR_MY_PUBKEY,
                "Invalid my public key hex.",
            )
        })?;
        my_pub_arr.copy_from_slice(&my_pub_vec);

        let expire_str = ca_res.identity.expire_at.as_ref().ok_or_else(|| {
            ApiError::new_system(
                ST_BAD_GATEWAY,
                ERR_INVALID_RESP,
                "Verified identity missing expire_at.",
            )
        })?;
        let expire_dt =
            NaiveDateTime::parse_from_str(expire_str, DATE_FORMAT_STANDARD).map_err(|_| {
                ApiError::new_system(
                    ST_BAD_GATEWAY,
                    ERR_INVALID_RESP,
        ...
        let dev_sig_bytes = hex::decode(sig_hex).map_err(|_| {
            ApiError::new_system(ST_BAD_GATEWAY, ERR_INVALID_SIG, "Invalid signature hex.")
        })?;
        ...
        if !verify_signature(&ca_pub_arr, &dev_msg, &dev_sig_struct).unwrap_or(false) {
            return Err(ApiError::new_system(
                ST_BAD_GATEWAY,
                ERR_SIG_FAIL,
                "Dev Signature verification failed (L2).",
            ));
        }
    }
