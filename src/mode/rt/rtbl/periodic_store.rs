...
use crate::{
    utils::db::DbPools,
    constants::PERIODICAL_STORE_INTERVAL_SEC,
    stt_config::ConfigManager,
    mode::rt::rtbl::identities_bl,
    mode::rt::rtutils::db_for_rt::DbPoolsExt,
};
...
pub fn start_periodical_store_task(db: Arc<DbPools>, config_manager: Arc<ConfigManager>) {
    tokio::spawn(async move {
        log::info!("<PeriodicStore> Background task started. Interval: {}s", PERIODICAL_STORE_INTERVAL_SEC);
        ...
        loop {
            ...
            let reliable_ca_list = identities_bl::select_reliable_ca_url_from_db(conn, &config_manager).await;
            ...
            log::info!("<PeriodicStore> Updated reliable CA cache with {} URLs.", list_len);
