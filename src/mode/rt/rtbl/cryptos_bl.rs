...
use crate::{
    ...
    utils::{
        ...
        jwt::{generate_token_for_vdr, JwtIDs, JwtRole, JwtUsr},
    },
};
use regex::Regex;
...
pub async fn encrypt_text(crypto_key: &str, text: String) -> Result<EncryptRes, ApiError> {
    ...
    let data = encrypt(&text, crypto_key).map_err(|e| {
        ApiError::new_system(
            ST_BAD_REQUEST,
            rterr::ERR_INVALID_REQUEST,
            format!("Failed: {}", e),
...
pub async fn decrypt_text(crypto_key: &str, text: String) -> Result<DecryptRes, ApiError> {
    ...
    let data = decrypt(&text, crypto_key).map_err(|e| {
        ApiError::new_system(
            ST_BAD_REQUEST,
            rterr::ERR_INVALID_REQUEST,
            format!("Failed: {}", e),
...
pub async fn create_vdr_token(
    ...
    let usr = usrs::Entity::find()
        ...
        .ok_or_else(|| {
            ApiError::new_system(ST_NOT_FOUND, rterr::ERR_NOT_FOUND, "VDR not found.")
        })?;
    // Generate 100-year token (876000 hours)
    let token = generate_token_for_vdr(skey, apx_id, vdr_id, usr.email, 876000).map_err(|e| {
        ApiError::new_system(
            ST_INTERNAL_SERVER_ERROR,
    ...
    // Encrypt token
    let encrypted_token = encrypt(&token, crypto_key).map_err(|e| {
        ApiError::new_system(
            ST_INTERNAL_SERVER_ERROR,
            rterr::ERR_UNEXPECTED,
    ...
    if let Some(record) = existing {
        ...
        if record.apx_id != Some(apx_id as i32) || record.vdr_id != Some(vdr_id as i32) {
            return Err(ApiError::new_system(
                ST_FORBIDDEN,
                rterr::ERR_AUTH,
                "This key is already owned by another entity. Ownership theft is prohibited.",
            ));
        }
        ...
        active.update(conn).await.map_err(|e| {
            ApiError::new_system(
                ST_INTERNAL_SERVER_ERROR,
                rterr::ERR_DATABASE,
                format!("Failed to update crypto: {}", e),
            )
        })?;
    } else {
        // Insert new record
        ...
        model.insert(conn).await.map_err(|e| {
            ApiError::new_system(
                ST_INTERNAL_SERVER_ERROR,
                rterr::ERR_DATABASE,
                format!("Failed to save crypto: {}", e),
            )
        })?;
    }
...
pub async fn get_vdr_token(
    ...
    let crypto = cryptos::Entity::find()
        ...
        .ok_or_else(|| {
            ApiError::new_system(ST_NOT_FOUND, rterr::ERR_NOT_FOUND, "Record not found.")
        })?;
