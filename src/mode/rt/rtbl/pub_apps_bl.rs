...
use crate::{
    ...
    mode::rt::{
        rtres::{
            errs_res::ApiError,
            pub_apps_res::{ListAppsPubRes, AppInfoPubItemRes},
        },
        rterr::rterr,
    },
};
...
) -> Result<ListAppsPubRes, ApiError> {
    log::debug!("<PubApps> list_apps_pub called.");

    let apps_records = apps::Entity::find()
        .all(conn)
        .await
        .map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string()))?;

    let items = apps_records.into_iter().map(|a| {
        // マニフェストデータの正規化 (Struct -> JSON) を通じて、
        // 署名検証時と同一のシリアライズ結果を保証する。
        let manifest_val = a.manifest_data.clone();
        let normalized_manifest = manifest_val.and_then(|v| {
            let m: Result<crate::utils::pkg_bl::MyCuteManifest, _> = serde_json::from_value(v);
            m.ok()
        ...
        AppInfoPubItemRes {
            ...
            description: normalized_manifest.as_ref().and_then(|m| m.get("description")).and_then(|v| v.as_str()).map(|s| s.to_string()), 
