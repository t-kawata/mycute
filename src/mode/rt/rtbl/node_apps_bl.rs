use crate::constants::{
    ERR_INSUFFICIENT_FUNDS, ST_BAD_GATEWAY, ST_BAD_REQUEST, ST_INTERNAL_SERVER_ERROR,
};
use crate::mode::rt::rtutils::db_for_rt::DbPoolsExt;
use crate::{
    constants::{
        APP_BUILD_DEFAULT_FILENAME, APP_BUILD_DIST_DIRNAME, APP_BUILD_FILE_EXTENSION,
        APP_BUILD_TEMP_DIR_PREFIX, APP_BUILD_WORK_DIRNAME, APP_INSTALL_PACKAGE_FILENAME,
        APP_INSTALL_TEMP_DIR_PREFIX, APP_MANIFEST_FILENAME, APP_TEMP_EXTRACT_DIRNAME,
        APP_VERIFY_PACKAGE_FILENAME, APP_VERIFY_TEMP_DIR_PREFIX, ED448_KEY_BYTES_LEN,
        ED448_SIGNATURE_BYTES_LEN, ERR_BUILD_FAILED, ERR_CA_ERROR, ERR_CA_UNREACHABLE,
        ERR_EXTRACT_FAILED, ERR_EXTRACT_ZIP, ERR_INSTALL_IO, ERR_INVALID_CA_RESPONSE,
        ERR_INVALID_ZIP, ERR_IO, ERR_PARSE_MANIFEST, ERR_READ_MANIFEST, ERR_READ_OUTPUT,
        ERR_REMOVE_OLD, ERR_SAVE, ERR_SIGN, ERR_WRITE_PKG, MYCUTE_APPS_DIRNAME,
        MYCUTE_DATA_DIRNAME, PATH_CA_APPS_ADVERTISE, PATH_CA_APPS_DISCOVER, PATH_CA_APPS_VOTE,
    },
    ...
    mode::rt::{
        ...
        rtres::{
            ...
            errs_res::ApiError,
            node_apps_res::{
                AdvertiseAppNodeRes, AppInfoNodeItemRes, AppInfoNodeRes, DiscoverAppNodeRes,
                VerifyAppNodeRes, VoteAppNodeRes,
            },
        },
        rtutils::{
            apps_types::AppLayer,
...
pub async fn build_app_node(
    ...
    log::info!(
        "<Apps> build_app (Node): zip_len={} bytes, filename={}",
        zip_data.len(),
        original_filename
    );
    ...
    zip.extract(&work_dir).map_err(|e| {
        ApiError::new_system(
            ST_BAD_REQUEST,
            ERR_EXTRACT_ZIP,
    ...
    for v in ver_recs {
        ...
        if let (Some(sig), Some(tok), Some(_ver_at), Some(exp_at)) =
            (v.signature, v.ca_token, v.verified_at, v.expire_at)
        {
    ...
    if !manifest_path.exists() {
        return Err(ApiError::new_system(
            ...
            format!("{} not found in zip root.", APP_MANIFEST_FILENAME),
        ));
    }
    ...
    let manifest: MyCuteManifest = serde_json::from_str(&manifest_content).map_err(|_| {
        ApiError::new_system(
            ST_BAD_REQUEST,
            ERR_PARSE_MANIFEST,
    ...
    let output_filename = if !original_filename.is_empty() {
        ...
        path.with_extension(APP_BUILD_FILE_EXTENSION)
            .file_name()
            .unwrap_or(std::ffi::OsStr::new(APP_BUILD_DEFAULT_FILENAME))
            .to_string_lossy()
            .to_string()
    } else {
        format!(
            "{}.{}.{}",
            manifest.name, manifest.global_app_version, APP_BUILD_FILE_EXTENSION
        )
    };
    ...
    pkg_bl::build_package(&config_manager, &work_dir, &output_path, credentials).map_err(
        |e: anyhow::Error| {
            ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_BUILD_FAILED, e.to_string())
        },
    )?;

    // 6. 成果物の読み込み
    let binary_data = std::fs::read(&output_path).map_err(|e| {
        ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_READ_OUTPUT, e.to_string())
    })?;
...
pub async fn install_app_file_node(
    ...
    log::info!(
        "<Apps> install_app_file_node: size={} bytes",
        package_data.len()
    );

    // 1. パッケージの展開とマニフェスト取得 (共通プロセス)
    let (_temp_dir, manifest, app_verification) = extract_package_to_temp(
        &package_data,
    ...
    if install_dir.exists() {
        std::fs::remove_dir_all(&install_dir).map_err(|e| {
            ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_REMOVE_OLD, e.to_string())
        })?;
    }
    std::fs::create_dir_all(&apps_root).ok();

    if let Err(_) = std::fs::rename(&extract_dir, &install_dir) {
        copy_dir_recursive(&extract_dir, &install_dir).map_err(|e| {
            ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_INSTALL_IO, e.to_string())
        })?;
    }
    ...
    let existing = apps::Entity::find()
        ...
        .filter(
            apps::Column::GlobalAppId
                .eq(uuid::Uuid::parse_str(&manifest.global_app_id).unwrap_or_default()),
        )
        .one(conn)
        .await
        .map_err(|e: sea_orm::DbErr| {
    ...
    if let Some(record) = existing {
        ...
        active.update(conn).await.map_err(|e: sea_orm::DbErr| {
            ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string())
        })?;
    } else {
        ...
        model.insert(conn).await.map_err(|e: sea_orm::DbErr| {
            ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string())
        })?;
    }

    // 7. レスポンスの構築
    let trust = AppTrustInfo::from_manifest(manifest.clone(), app_verification);
...
pub async fn verify_app_node(
    ...
    let trust = AppTrustInfo::from_manifest(manifest, app_verification);
...
pub async fn discover_app_node(
    client: &SecureClient,
    req: DiscoverAppNodeReq,
) -> Result<DiscoverAppNodeRes, ApiError> {
    log::debug!(
        "<Apps> discover_app_node: Proxying to CA: {} for IDs: {:?}, query: {:?}",
        req.ca_base_url,
        req.app_ids,
        req.query
    );
    ...
    let url = format!(
        "{}{}",
        req.ca_base_url.trim_end_matches('/'),
        PATH_CA_APPS_DISCOVER
    );
    ...
    let ca_res: DiscoverAppCaRes = res.json().await.map_err(|e| {
        ApiError::new_system(ST_BAD_GATEWAY, ERR_INVALID_CA_RESPONSE, e.to_string())
    })?;

    Ok(DiscoverAppNodeRes {
        items: ca_res.items,
    })
}
...
pub async fn advertise_app_node(
    client: &SecureClient,
    req: AdvertiseAppNodeReq,
) -> Result<AdvertiseAppNodeRes, ApiError> {
    log::debug!(
        "<Apps> advertise_app_node: Proxying to CA: {} for app: {}",
        req.ca_base_url,
        req.app_id
    );
    ...
    let url = format!(
        "{}{}",
        req.ca_base_url.trim_end_matches('/'),
        PATH_CA_APPS_ADVERTISE
    );

    let ca_req = AdvertiseAppCaReq { app_id: req.app_id };
    ...
    let ca_res: AdvertiseAppCaRes = res.json().await.map_err(|e| {
        ApiError::new_system(ST_BAD_GATEWAY, ERR_INVALID_CA_RESPONSE, e.to_string())
    })?;
...
pub async fn vote_app_node(
    ...
    log::debug!(
        "<Apps> vote_app_node: Processing vote for app: {}, value: {}, ca: {}",
        req.app_id,
        req.vote,
        req.ca_base_url
    );
    ...
    let ticket_record = tickets::Entity::find()
        ...
        .ok_or_else(|| {
            ApiError::new_system(
                ST_BAD_REQUEST,
                rterr::ERR_NOT_FOUND,
                "Ticket not found for this forum.",
            )
        })?;
    ...
    let ticket_payload: identities_bl::TicketPayload = serde_json::from_value(ticket_json.clone())
        .map_err(|e| {
            ApiError::new_system(
                ST_INTERNAL_SERVER_ERROR,
                rterr::ERR_INVALID_REQUEST,
    ...
    if vote_allocated < 0 {
        return Err(ApiError::new_system(
            ST_INTERNAL_SERVER_ERROR,
            rterr::ERR_INVALID_REQUEST,
            "Calculated vote_allocated is negative. Data inconsistency detected.",
        ));
    }
    ...
    let ca_payload_str = format_vote_payload(
        &req.app_id,
        req.vote,
        vote_allocated,
        &timestamp.to_string(),
    ...
    let sig_struct = my_keypair
        .sign(ca_payload_str.as_bytes())
    ...
    let ca_res: VoteAppCaRes = res.json().await.map_err(|e| {
        ApiError::new_system(ST_BAD_GATEWAY, ERR_INVALID_CA_RESPONSE, e.to_string())
    })?;
    ...
    let ca_res_payload =
        format_vote_receipt_payload(ca_res.vote_allocated, ca_res.timestamp, &signature_hex);
    ...
    let ca_sig_struct = crypto::Ed448Signature {
        signature: ca_sig_arr,
    };

    let sig_valid =
        crypto::verify_signature(&ca_pub_arr, ca_res_payload.as_bytes(), &ca_sig_struct)
            .unwrap_or(false);
    ...
    if !sig_valid || !amount_match {
        ...
        tokio::spawn(async move {
            if let Err(e) =
                report_crime_broadcast(&db_clone, &config_clone, &client_clone, &ev, None).await
            {
                log::warn!(
                    "<Apps> Failed to broadcast CaVoteAllocatedFraud report: {}",
                    e
                );
            }
        });
        ...
        return Err(ApiError::new_system(
            ST_BAD_GATEWAY,
            ERR_INVALID_CA_RESPONSE,
            "CA Fraud detected: Invalid signature or vote_allocated mismatch.",
        ));
    }
    ...
    if diff > 0 {
        log::info!(
            "<Apps> Vote success for CA '{}'. Allocated {} balance. Remaining: {}",
            req.ca_base_url,
            diff,
            new_balance
        );
        ...
        log::info!(
            "<Apps> Vote success for CA '{}'. Refunded {} balance. Remaining: {}",
            req.ca_base_url,
            -diff,
            new_balance
        );
    ...
    Ok(VoteAppNodeRes { success: true })
}
...
pub fn load_my_rem_payload_for_entry(
    ...
    let rem_dec =
        crypto::decrypt(rem_enc, crypto_key).map_err(|e| format!("Decrypt failed: {}", e))?;
...
> {
    ...
    std::fs::write(&pkg_path, package_data).map_err(|e| {
        ApiError::new_system(
            ST_INTERNAL_SERVER_ERROR,
            ERR_WRITE_PKG,
    ...
    // 4. 展開と検証
    let (manifest, app_verification) =
        pkg_bl::extract_package(config_manager, &pkg_path, &extract_dir).map_err(|e| {
            ApiError::new_system(
                ST_BAD_REQUEST,
                ERR_EXTRACT_FAILED,
                format!("Package extraction failed: {}", e),
            )
        })?;
