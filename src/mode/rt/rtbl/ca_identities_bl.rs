use sea_orm::{DatabaseConnection, EntityTrait, QueryFilter, ColumnTrait, Set, ActiveModelTrait, PaginatorTrait, QueryOrder, QuerySelect, Condition, TransactionTrait, prelude::Expr};
use crate::constants::{ST_BAD_REQUEST, ST_NOT_FOUND, ST_INTERNAL_SERVER_ERROR, ST_CONFLICT};
use axum::http::StatusCode;
use chrono::Duration;
use crate::{
    entities::{identities, verifications, forums},
    mode::rt::{
        rtreq::ca_identities_req::{SearchIdentitiesCaReq, EntryIdentityCaReq, VerifyIdentityCaReq, ApplyIdentityCaReq},
        rtres::{
            errs_res::ApiError,
            ca_identities_res::{SearchIdentitiesCaRes, IdentityItemCaRes, GetIdentityCaRes, EntryIdentityCaRes, VerifyIdentityCaRes, DeleteIdentityCaRes, SyncIdentityCaRes, ApplyIdentityCaRes}
        },
        rterr::rterr,
        rtbl::identities_bl::{self},
    },
    utils::jwt::{JwtUsr, JwtIDs, JwtRole},
    stt_config::ConfigManager,
    utils::time,
    constants::{
        ED448_PUBKEY_HEX_LEN,
        APX_ID_ISOLATED, VDR_ID_ISOLATED, IS_CANDIDATE_FALSE, IS_CANDIDATE_TRUE,
        KEY_TICKET_SIGNATURE,
        ED448_KEY_BYTES_LEN,
        ERR_SIGN, ERR_CA_PUBKEY, ERR_INVALID_SIG, ERR_DECRYPT, ERR_EXPECTED_RECORD,
...
pub async fn search_identities(
    ...
    log::debug!("<CaIdentities> search_identities: apx {}, vdr {}", ids.apx_id, ids.vdr_id);
    ...
    for m in items {
        ...
        let (verified_at, expire_at, is_candidate, identity_layer) = if let Some(v) = verification {
            let layer = identities_bl::determine_layer(
                ...
                &m.public_key,
                &ca_pubkey,
                &config_manager.settings.read().server.my_base_url.clone().unwrap_or_default(),
    ...
    Ok(SearchIdentitiesCaRes { total, items: result_items })
}
...
pub async fn get_identity(
    ...
    let identity = find_ca_identities_base(ids.apx_id as i32, ids.vdr_id as i32)
        ...
        .await
        .map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string()))?
        .ok_or_else(|| ApiError::new_system(ST_NOT_FOUND, rterr::ERR_NOT_FOUND, "Identity not found."))?;
...
pub async fn entry_identity_ca(
    ...
    if req.public_key.len() != ED448_PUBKEY_HEX_LEN || !req.public_key.chars().all(|c| c.is_ascii_hexdigit()) {
        return Err(ApiError::new_system(ST_BAD_REQUEST, rterr::ERR_INVALID_REQUEST, "Invalid public_key."));
    }
    ...
    // 公開鍵をデコード
    let pub_bytes = hex::decode(&req.public_key).map_err(|_| ApiError::new_system(ST_BAD_REQUEST, rterr::ERR_INVALID_REQUEST, "Invalid public_key hex."))?;
    let mut pub_arr = [0u8; ED448_KEY_BYTES_LEN];
    pub_arr.copy_from_slice(&pub_bytes);

    // 署名をデコード
    let sig_bytes = hex::decode(&req.signature).map_err(|_| ApiError::new_system(ST_BAD_REQUEST, ERR_INVALID_SIG, "Invalid signature hex."))?;
    ...
    if !valid_pop {
        return Err(ApiError::new_system(ST_BAD_REQUEST, ERR_INVALID_SIG, "Invalid PoP signature."));
    }
    ...
    // トランザクション開始
    let identity = conn.transaction::<_, identities::Model, ApiError>(|txn| Box::pin(async move {
        // 2. 既存の確認または新規作成
        ...
        let (identity, _created_new) = if let Some(ext) = existing {
            ...
            let res = model.insert(txn).await.map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string()))?;
    ...
    if let Some(existing) = req.existing_forums {
        ...
        for f in existing {
            if let Ok(u) = uuid::Uuid::parse_str(&f.id) {
                ...
                if let Ok(node_ts) = chrono::NaiveDateTime::parse_from_str(&f.updated_at, crate::constants::DATE_FORMAT_STANDARD) {
    ...
    let (my_base_url, ca_token) = {
        ...
        let url = s.server.my_base_url.clone().expect("my_base_url must be configured at startup validation");
        
        let token = if let Some(enc) = &s.my_cat {
            // my_cat を復号
            let crypto_key = s.server.rt_crypto_key.clone();
            Some(crypto::decrypt(enc, &crypto_key).map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_DECRYPT, format!("Failed to decrypt my CA token: {}", e)))?)
        } else {
            None
        };
    ...
    for forum in all_forums {
        ...
        // チケットに署名
        let sig_struct = ca_keypair.sign(payload_canonical_json.as_bytes()).map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_SIGN, e.to_string()))?;
        let sig_hex = hex::encode(sig_struct.signature);

        // 最終的なチケット JSON (署名を含む)
        // 配布用 JSON オブジェクトの構築
        let mut ticket_map = serde_json::to_value(&ticket_payload_struct).map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, "ERR_TICKET_GEN", e.to_string()))?;
        if let Some(obj) = ticket_map.as_object_mut() {
            obj.insert(KEY_TICKET_SIGNATURE.to_string(), serde_json::Value::String(sig_hex));
        }
...
pub async fn apply_identity_ca(
    ...
    conn.transaction::<_, (), ApiError>(|txn| Box::pin(async move {
        if let Some(identity) = existing { // 既に存在する場合
            ...
            if let Some(v) = &verification {
                ...
                if v.verified_at.is_some() && !is_expired_or_near {
                    return Err(ApiError::new_system(ST_CONFLICT, rterr::ERR_DUPLICATE, "Already verified."));
                }
                // 既に申請中または申請完了の場合は再申請を許可しない
                if v.is_candidate != 0 && v.verified_at.is_none() {
                    return Err(ApiError::new_system(ST_CONFLICT, rterr::ERR_DUPLICATE, "Already applied and pending."));
                }
            }
...
pub async fn verify_identity_ca(
    ...
    let verification = verifications::Entity::find()
        ...
        .await
        .map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string()))?
        .ok_or_else(|| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_EXPECTED_RECORD, "Verification record not found. Please apply first."))?;
    ...
    let ca_token_to_imprint = if let Some(enc) = my_cat_enc {
        ...
        let decrypted = crypto::decrypt(&enc, &crypto_key).map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_DECRYPT, format!("Failed to decrypt my CA token: {}", e)))?;
...
pub async fn sync_identity_ca(
    ...
    let ca_pubkey = identities_bl::get_pubkey(config_manager.clone()).await.map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, ERR_CA_PUBKEY, e.to_string()))?;
    ...
    let v = verification.ok_or_else(|| {
        ApiError::new_system(StatusCode::ACCEPTED, ERR_VERIFICATION_PENDING, "Verification is still pending (no record).")
    })?;

    if v.signature.is_none() {
        return Err(ApiError::new_system(StatusCode::ACCEPTED, ERR_VERIFICATION_PENDING, "Verification is still pending (no signature)."));
    }
    ...
    let ca_base_url = config_manager.settings.read().server.my_base_url.clone().unwrap_or_default();
...
pub async fn delete_identity_ca(
    ...
    let is_authorized = match ju.role() {
        JwtRole::APX => (identity.apx_id == ids.apx_id as i32) || (identity.apx_id == 0 && identity.vdr_id == 0),
        JwtRole::VDR | JwtRole::USR => identity.apx_id == ids.apx_id as i32 && identity.vdr_id == ids.vdr_id as i32,
        _ => false,
    };

    if !is_authorized {
        return Err(ApiError::new_system(ST_NOT_FOUND, rterr::ERR_NOT_FOUND, "Not found."));
    }
    ...
    // トランザクション開始
    conn.transaction::<_, (), ApiError>(|txn| Box::pin(async move {
        identities::Entity::delete_by_id(identity.id).exec(txn).await
            .map_err(|e| ApiError::new_system(ST_INTERNAL_SERVER_ERROR, rterr::ERR_DATABASE, e.to_string()))?;
    ...
    Ok(DeleteIdentityCaRes { id: res_id, deleted: true })
}
