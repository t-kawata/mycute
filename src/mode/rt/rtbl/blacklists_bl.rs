use crate::{
    constants::{
        BLACKLIST_CLEANUP_MARGIN_HOURS, ED448_KEY_BYTES_LEN, ED448_SIGNATURE_BYTES_LEN, ERR_DB,
        ERR_DECODE, ERR_INVALID_KEY, ERR_INVALID_SIG, ERR_LOW_LEVEL, ERR_SIG_FAIL,
        PATH_BLACKLISTS_REPORT, PATH_BLACKLISTS_SYNC, TIMESTAMP_TOLERANCE_MS,
    },
    entities::blacklists,
    mode::rt::{
...
pub fn check_evidence_structure(evidence: &CrimeEvidence) -> Result<(), ApiError> {
    ...
    let msg_raw = hex::decode(&evidence.signed_payload).map_err(|e| {
        ApiError::new_system(
            StatusCode::BAD_REQUEST,
            ERR_DECODE,
            format!("Invalid signed_payload hex: {}", e),
        )
    })?;

    // 犯罪種別ごとのペイロード検証ロジック
    match &evidence.detail {
        CrimeDetail::TimestampFraud {
            wrong_timestamp, ..
        } => {
            ...
            if msg_raw != ts_bytes {
                return Err(ApiError::new_system(
                    StatusCode::BAD_REQUEST,
                    ERR_LOW_LEVEL,
                    "signed_payload do not match wrong_timestamp",
                ));
            }
        }
        CrimeDetail::BudgetFraud {
            reported_vote_allocated,
            ..
        } => {
            ...
            match String::from_utf8(msg_raw.clone()) {
                ...
                Err(_) => {
                    ...
                    return Err(ApiError::new_system(
                        StatusCode::BAD_REQUEST,
                        ERR_DECODE,
                        "signed_payload is not valid utf8 for BudgetFraud",
                    ));
                }
            }
        }
        CrimeDetail::CaVoteAllocatedFraud {
            original_vote_allocated,
            tampered_vote_allocated,
            original_payload,
            ..
        } => {
            ...
            match String::from_utf8(msg_raw.clone()) {
                ...
                Err(_) => {
                    return Err(ApiError::new_system(
                        StatusCode::BAD_REQUEST,
                        ERR_DECODE,
                        "signed_payload is not valid utf8 for CaVoteAllocatedFraud",
                    ));
                }
            }
        }
        ...
        CrimeDetail::TimestampFraud {
            ...
            if diff <= TIMESTAMP_TOLERANCE_MS {
                return Err(ApiError::new_system(
                    ...
                    ERR_LOW_LEVEL,
                    format!(
                        "Time difference {}ms is within tolerance (Not a crime)",
                        diff
                    ),
                ));
            }
        ...
        CrimeDetail::BudgetFraud {
            ...
            if *reported_vote_allocated == *expected_vote_allocated {
                return Err(ApiError::new_system(
                    StatusCode::BAD_REQUEST,
                    ERR_LOW_LEVEL,
                    "Reported and expected vote_allocated match (Not a crime)",
                ));
            }
        }
        CrimeDetail::CaVoteAllocatedFraud {
            ...
            if *original_vote_allocated == *tampered_vote_allocated {
                return Err(ApiError::new_system(
                    StatusCode::BAD_REQUEST,
                    ERR_LOW_LEVEL,
                    "Original and tampered vote_allocated match (Not a crime)",
                ));
            }
        }
    }
    ...
    let existing = blacklists::Entity::find()
        ...
        .map_err(|e| {
            ApiError::new_system(
                StatusCode::INTERNAL_SERVER_ERROR,
                ERR_DB,
                format!("Failed to find blacklist: {}", e),
            )
        })?;
    ...
    if let Some(model) = existing {
        ...
        active.insert(&txn).await.map_err(|e| {
            ApiError::new_system(
                StatusCode::INTERNAL_SERVER_ERROR,
                ERR_DB,
                format!("Failed to insert blacklist: {}", e),
            )
        })?;
    }
...
pub async fn delete_expired_blacklists(db: &DbPools) -> Result<u64, ApiError> {
    ...
    let result = blacklists::Entity::delete_many()
        .filter(
            Expr::col(blacklists::Column::ObservedAt).lt(Expr::val(now).sub(
                Expr::col(blacklists::Column::PrisonTermHours)
                    .mul(3_600_000)
                    .add(margin_ms),
            )),
        )
        ...
        .map_err(|e| {
            ApiError::new_system(
                StatusCode::INTERNAL_SERVER_ERROR,
                ERR_DB,
                format!("Failed to cleanup expired blacklists: {}", e),
            )
        })?;

    if result.rows_affected > 0 {
        log::info!(
            "<Blacklist> Cleaned up {} expired blacklist records (SQL optimized).",
...
pub async fn is_blacklisted(db: &DbPools, pubkey: &str) -> Result<bool, ApiError> {
    ...
    let count = blacklists::Entity::find()
        ...
        .filter(
            Expr::val(now)
                .sub(Expr::col(blacklists::Column::ObservedAt))
                .lt(Expr::col(blacklists::Column::PrisonTermHours).mul(3_600_000)),
        )
        .count(conn)
        .await
        .map_err(|e| {
            ApiError::new_system(
                StatusCode::INTERNAL_SERVER_ERROR,
                ERR_DB,
                format!("Failed to check blacklisted: {}", e),
            )
        })?;
...
pub async fn get_blacklists_since(
    db: &DbPools,
    since_ts: i64,
) -> Result<Vec<blacklists::Model>, ApiError> {
    let conn = db.get_ro_for_rt()?;
    blacklists::Entity::find()
        ...
        .map_err(|e| {
            ApiError::new_system(
                StatusCode::INTERNAL_SERVER_ERROR,
                ERR_DB,
                format!("Failed to get blacklists since {}: {}", since_ts, e),
            )
        })
}
...
pub async fn get_blacklist_all(db: &DbPools) -> Result<Vec<blacklists::Model>, ApiError> {
    ...
    blacklists::Entity::find().all(conn).await.map_err(|e| {
        ApiError::new_system(
            StatusCode::INTERNAL_SERVER_ERROR,
            ERR_DB,
            format!("Failed to get all blacklists: {}", e),
        )
    })
}
...
pub async fn report_crime_to_ca(
    ...
    let url = format!(
        "{}{}",
        ca_base_url.trim_end_matches('/'),
        PATH_BLACKLISTS_REPORT
    );
    log::debug!(
        "<Blacklist> Reporting crime to CA: {} -> target: {}",
        url,
        evidence.target_pubkey
    );
    ...
    client
        .post_ca::<serde_json::Value, _, _>(&url, &req)
        .await
        .map(|_| ())
        .map_err(|e| {
            ApiError::new_system(
                ...
                "CA_REPORT_FAIL",
                format!("Failed to report to CA: {}", e),
            )
        })
}
...
pub async fn report_crime_broadcast(
    ...
    // 1. 自分の信頼する CA リストを取得
    let mut target_urls = identities_bl::get_reliable_ca_urls(conn, config)
        .await
        .unwrap_or_default();
    ...
    for url in target_urls {
        match report_crime_to_ca(evidence, &url, client).await {
            ...
            Err(e) => {
                log::warn!(
                    "<Blacklist> Failed to broadcast crime report to {}: {}",
                    url,
                    e
                );
            }
    ...
    if success_count == 0 {
        ...
        log::warn!(
            "<Blacklist> Broadcast report yielded 0 successes for target: {}",
            evidence.target_pubkey
        );
    } else {
        log::info!(
            "<Blacklist> Broadcast report succeeded to {} CAs for target: {}",
            success_count,
            evidence.target_pubkey
        );
    }
...
pub async fn sync_blacklists_with_ca(
    ...
    log::debug!(
        "<Blacklist> Syncing blacklists with CA: {} since {}",
        url,
        last_sync_ts
    );
    ...
    let res: SyncBlacklistCaRes = match client.post_ca(&url, &req).await {
        ...
        Err(e) => {
            ...
            return Err(ApiError::new_system(
                StatusCode::BAD_GATEWAY,
                "CA_SYNC_FAIL",
                format!("Failed to fetch from CA: {}", e),
            ));
        }
    };
