...
pub async fn report_blacklist_node(
    ...
    log::info!(
        "<Blacklist> Processing report_blacklist_node. Target: {}",
...
pub async fn sync_blacklists_node(
    ...
    if !req.ca_base_url.is_empty() {
        match blacklists_bl::sync_blacklists_with_ca(db, &req.ca_base_url, client, config_manager)
            .await
        {
            Ok(count) => {
            ...
            Err(e) => {
                log::warn!(
                    "<Blacklist> Failed to sync with CA '{}': {}",
                    req.ca_base_url,
                    e
                );
                // 同期指示自体は受け付けたが中身が失敗したケース。
                // 厳密にエラーを返すべきかは要検討だが、一旦 false を返す設計も可能。
                return Ok(SyncBlacklistNodeRes {
                    success: false,
                    new_items_count: 0,
                });
            }
