...
use crate::constants::{
    ...
    TAG_MARKER_P2P_STRICT, TAG_MARKER_P2P_OPTIONAL,
};
use crate::mode::rt::rterr::rterr::{ERR_TIME_SKEW, ERR_BLACKLISTED, ERR_AUTH};
use crate::mode::rt::rtres::errs_res::ApiError;
use crate::mode::rt::rtbl::{identities_bl, blacklists_bl};
...
fn get_p2p_path_sets() -> (&'static HashSet<String>, &'static HashSet<String>) {
    let (strict, optional) = P2P_PATH_SETS.get_or_init(|| {
        ...
        let openapi = crate::mode::rt::req_map::RUNTIME_OPENAPI.get().unwrap_or(&binding);

        for (path, path_item) in &openapi.paths.paths {
            let ops = [
                &path_item.get, &path_item.post, &path_item.put, &path_item.delete,
                &path_item.options, &path_item.head, &path_item.patch, &path_item.trace,
            ];
            for op_opt in ops {
                if let Some(operation) = op_opt {
                    if let Some(tags) = &operation.tags {
                        let prefix = path.split('{').next().unwrap_or(&path);
                        if tags.iter().any(|t: &String| t.contains(TAG_MARKER_P2P_STRICT)) {
                            strict_set.insert(prefix.to_string());
                        } else if tags.iter().any(|t: &String| t.contains(TAG_MARKER_P2P_OPTIONAL)) {
...
pub async fn p2p_clock_sync_enforcement_middleware(
    ...
    if is_strict {
        ...
        if h_ts.is_none() || h_sig.is_none() || h_pub.is_none() || h_ca.is_none() {
            log::warn!("P2P Middleware: Missing mandatory headers on STRICT path: {}", path);
        ...
        if !peer_ca_url_list.is_empty() {
            ...
            tokio::spawn(async move {
                for url in selected_urls {
                    if let Err(e) = blacklists_bl::sync_blacklists_with_ca(&db_clone, &url, &client_clone, &config_clone).await {
                        log::warn!("P2P Clock Sync Enforcement: Background sync with CA {} failed: {}", url, e.to_string());
                    }
                }
            });
        }
    ...
    // 2.4 P2P 検証 (ヘッダーが揃っている前提)
    {
        ...
        if let Ok(true) = blacklists_bl::is_blacklisted(&db, peer_pub_hex).await {
            log::warn!("P2P Clock Sync Enforcement: Disconnected request from blacklisted node: {}", peer_pub_hex);
        ...
        if let Err(e) = blacklists_bl::check_evidence_structure(&evidence) {
            log::warn!("P2P Clock Sync Enforcement: Invalid signature from {}. Error: {}", peer_pub_hex, e);
        ...
        if diff > TIMESTAMP_TOLERANCE_MS { // タイムスタンプの誤差が許容範囲を超えていたら
            log::warn!("P2P Clock Sync Enforcement: Time skew detected. Peer: {}, My: {}, Diff: {}", peer_ts, my_ts, diff);
            ...
            // 司法 (Judgment): L3 ノードのみ CA へ報告 (Broadcast)
            {
                ...
                let sender_ca_urls: Option<Vec<String>> = h_ca
                    .and_then(|v| v.to_str().ok())
                    .map(|s| s.split(',').map(|u| u.trim().to_string()).filter(|u| !u.is_empty()).collect());

                tokio::spawn(async move {
                    if let Err(e) = blacklists_bl::report_crime_broadcast(&db_clone, &config_clone, &client_clone, &evidence_clone, sender_ca_urls).await {
...
async fn inject_headers(mut res: Response<Body>, config: &Arc<ConfigManager>, db: &Arc<DbPools>) -> Response<Body> {
