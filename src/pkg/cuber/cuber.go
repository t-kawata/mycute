...
import (
	...
	"mycute/pkg/cuber/tasks/chunking"
	"mycute/pkg/cuber/tasks/graph"
	"mycute/pkg/cuber/tasks/ingestion"
	"mycute/pkg/cuber/tasks/memify"
	"mycute/pkg/cuber/tasks/metacognition"
	storageTaskPkg "mycute/pkg/cuber/tasks/storage"
	"mycute/pkg/cuber/tasks/summarization"
...
func NewCuberService(config CuberConfig) (*CuberService, error) {
	...
	log.Printf("[Cuber] Initializing KuzuDB at %s", config.KuzuDBDatabasePath)
...
// Absorb は、ファイルをCuberシステムに取り込み、知識グラフを構築する統合関数です。
...
// 返り値:
//   - error: エラーが発生した場合
func (s *CuberService) Absorb(ctx context.Context, filePaths []string, dataset string, user string) error {
	// ========================================
	// 1. ファイルの取り込み（add）
	// ========================================
...
func (s *CuberService) add(ctx context.Context, filePaths []string, dataset string, user string) error {
...
//
// 注意: このメソッドはパッケージ内部で使用されます。外部からはAbsorbを使用してください。
func (s *CuberService) cognify(ctx context.Context, dataset string, user string) error {
	// グループIDを生成
	groupID := user + "-" + dataset
	...
	// 1. 事前初期化されたLLMを使用
	// ========================================
	// s.LLM は NewCuberService で初期化済み
...
func (s *CuberService) Search(ctx context.Context, query string, searchType search.SearchType, dataset string, user string) (string, error) {
	// グループIDを生成
	groupID := user + "-" + dataset
...
// 返り値:
//   - error: エラーが発生した場合
func (s *CuberService) Memify(ctx context.Context, dataset string, user string, config *MemifyConfig) error {
	if config == nil {
...
func (s *CuberService) executeMemifyCore(ctx context.Context, dataset string, user string, config *MemifyConfig) error {
	groupID := user + "-" + dataset
	if config.RulesNodeSetName == "" {
		config.RulesNodeSetName = "coding_agent_rules"
	}
...
func (s *CuberService) attemptToResolveUnknown(ctx context.Context, unknown *metacognition.Unknown, groupID string) error {
