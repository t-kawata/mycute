...
pub fn install(home: &Path) -> Result<PathBuf> {
    let asset = get_bifrost_asset().ok_or_else(|| {
        BifrostError::UnsupportedPlatform("No Bifrost asset available for this platform".to_string())
    })?;

    let install_dir = home.join(BIFROST_DIRNAME).join(asset.version);
    
    // すでに展開済みの場合はスキップ
    if install_dir.exists() {
        log::info!("Bifrost {} is already installed at {:?}", asset.version, install_dir);
...
fn remove_quarantine_flag(install_dir: &Path) -> Result<()> {
    log::info!("Removing macOS quarantine flag from Bifrost at {:?}", install_dir);
