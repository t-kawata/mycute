...
`make build` が正常終了すれば、ビルド環境は整っている。

#### 音声入力の確認

Windows で音声入力（Alt ダブルタップ）を使用するには、以下の追加設定が必要である：

1. **設定 → 時刻と言語 → 言語と地域** を開く
   - 使用する言語（日本語など）の横にある「…」→ **言語オプション** を開く
   - 「音声認識」モジュールが一覧にあればインストールされていなければダウンロードする
   - このモジュールがないと音声認識エンジン自体が機能しない

2. **設定 → プライバシーとセキュリティ → 音声認識** を開く
   - 「音声認識」のスイッチを**オン**にする

3. **設定 → プライバシーとセキュリティ → マイク** を開く
   - 「デスクトップアプリのマイクアクセスを許可する」が**オン**であることを確認する

これらが未設定の場合、Alt ダブルタップで起動音は鳴るが音声が認識されない（音声認識モデル未インストール、または権限不足により WinRT AudioGraph がサイレントに失敗する）。
