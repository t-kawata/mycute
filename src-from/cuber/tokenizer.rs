...
use lindera::tokenizer::{Tokenizer as LinderaTokenizerInner, TokenizerBuilder};
use serde_json::json;
...
impl LinderaTokenizer {
    ...
    pub fn new() -> Result<Self, CuberError> {
        ...
        let inner = TokenizerBuilder::from_config(config)
            .map_err(|e| CuberError::TokenizerInitError(format!("Failed to create TokenizerBuilder: {}", e)))?
            .build()
            .map_err(|e| CuberError::TokenizerInitError(format!("Failed to build tokenizer: {}", e)))?;
