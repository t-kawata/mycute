...
pub struct CuberConfig {
    ...
    // TODO: Phase 5 以降で追加する LLM プロバイダー設定
    // TODO: Phase 6 以降で追加する Memify 関連設定
}
...
impl CuberConfig {
    /// 環境変数から設定を読み込む
    ///
    /// .env ファイルまたはシステム環境変数から `CUBER_*` プレフィックスの変数を読み込みます。
    pub fn from_env() -> Self {
        let mut config = Self::default();

        if let Ok(v) = std::env::var("DB_DIR_PATH") {
            config.db_dir_path = v;
        }
        if let Ok(v) = std::env::var("CUBER_STORAGE_IDLE_TIMEOUT_MINUTES") {
            config.storage_idle_timeout_minutes = v.parse().unwrap_or(config.storage_idle_timeout_minutes);
        }
        if let Ok(v) = std::env::var("CUBER_S3_CLEANUP_INTERVAL_MINUTES") {
            config.s3_cleanup_interval_minutes = v.parse().unwrap_or(config.s3_cleanup_interval_minutes);
        }
        if let Ok(v) = std::env::var("CUBER_S3_RETENTION_HOURS") {
            config.s3_retention_hours = v.parse().unwrap_or(config.s3_retention_hours);
        }
        if let Ok(v) = std::env::var("CUBER_META_SIMILARITY_THRESHOLD_UNKNOWN") {
            config.meta_similarity_threshold_unknown = v.parse().unwrap_or(config.meta_similarity_threshold_unknown);
        }
        if let Ok(v) = std::env::var("CUBER_META_SIMILARITY_THRESHOLD_REFLECTION") {
            config.meta_similarity_threshold_reflection = v.parse().unwrap_or(config.meta_similarity_threshold_reflection);
        }
        if let Ok(v) = std::env::var("CUBER_META_SIMILARITY_THRESHOLD_CRYSTALLIZATION") {
            config.meta_similarity_threshold_crystallization = v.parse().unwrap_or(config.meta_similarity_threshold_crystallization);
        }
        if let Ok(v) = std::env::var("CUBER_META_SEARCH_LIMIT_UNKNOWN") {
            config.meta_search_limit_unknown = v.parse().unwrap_or(config.meta_search_limit_unknown);
        }
        if let Ok(v) = std::env::var("CUBER_META_SEARCH_LIMIT_REFLECTION_CHUNK") {
            config.meta_search_limit_reflection_chunk = v.parse().unwrap_or(config.meta_search_limit_reflection_chunk);
        }
        if let Ok(v) = std::env::var("CUBER_META_SEARCH_LIMIT_REFLECTION_RULE") {
            config.meta_search_limit_reflection_rule = v.parse().unwrap_or(config.meta_search_limit_reflection_rule);
        }
        if let Ok(v) = std::env::var("CUBER_META_CRYSTALLIZATION_MIN_CLUSTER") {
            config.meta_crystallization_min_cluster = v.parse().unwrap_or(config.meta_crystallization_min_cluster);
        }
        if let Ok(v) = std::env::var("CUBER_GRAPH_METABOLISM_ALPHA") {
            config.graph_metabolism_alpha = v.parse().unwrap_or(config.graph_metabolism_alpha);
        }
        if let Ok(v) = std::env::var("CUBER_GRAPH_METABOLISM_DELTA") {
            config.graph_metabolism_delta = v.parse().unwrap_or(config.graph_metabolism_delta);
        }
        if let Ok(v) = std::env::var("CUBER_GRAPH_METABOLISM_PRUNE_THRESHOLD") {
            config.graph_metabolism_prune_threshold = v.parse().unwrap_or(config.graph_metabolism_prune_threshold);
        }
        if let Ok(v) = std::env::var("CUBER_GRAPH_PRUNING_GRACE_PERIOD_MINUTES") {
            config.graph_pruning_grace_period_minutes = v.parse().unwrap_or(config.graph_pruning_grace_period_minutes);
        }
        if let Ok(v) = std::env::var("CUBER_DEBUG") {
